...
export async function GET(request: NextRequest): Promise<Response> {
  ...
  if (!customerId) {
    return NextResponse.json(
      { ok: false, error: "google_ads_not_configured" },
      { status: 400 },
    );
  }
  ...
  const selectedCampaignId = campaignIdRaw
    ? normalizeDigits(campaignIdRaw)
    : "";
  ...
  const db = getDb();
  const [totalsRow, topCampaigns, topSearchTerms, conversionActionRows] =
    await Promise.all([
      db
        .select({
          impressions:
          ...
          days: sql<number>`count(distinct ${googleAdsInsightsDaily.dateStart})`.mapWith(
            Number,
          ),
        })
        ...
        .where(
          and(
            ...
            ...(hasCampaignScope
              ? [eq(googleAdsInsightsDaily.campaignId, selectedCampaignId)]
              : []),
          ),
        )
      ...
      db
        ...
        .orderBy(
          desc(sql`sum(${googleAdsInsightsDaily.conversions})`),
          desc(sql`sum(${googleAdsInsightsDaily.cost})`),
        )
        .limit(20),
      db
        .select({
        ...
        .where(
          and(
            ...
            ...(hasCampaignScope
              ? [eq(googleAdsSearchTermsDaily.campaignId, selectedCampaignId)]
              : []),
          ),
        )
        .groupBy(
  ...
  const selectedCampaignName = hasCampaignScope
    ? (campaignNameById.get(selectedCampaignId) ?? null)
    : null;
  ...
  const callConversionActions = conversionActions
    .filter((row) => row.cls === "call")
    .map((row) => row.name)
    .filter((name): name is string => typeof name === "string");
  const bookingConversionActions = conversionActions
    .filter((row) => row.cls === "booking")
    .map((row) => row.name)
    .filter((name): name is string => typeof name === "string");
