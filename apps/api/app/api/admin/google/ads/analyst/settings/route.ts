...
import {
  getGoogleAdsAnalystPolicy,
  DEFAULT_GOOGLE_ADS_ANALYST_POLICY,
} from "@/lib/policy";
...
export async function POST(request: NextRequest): Promise<Response> {
  ...
  const next = {
    enabled:
      typeof payload["enabled"] === "boolean"
        ...
        : current.enabled,
    autonomous:
      typeof payload["autonomous"] === "boolean"
        ? payload["autonomous"]
        : current.autonomous,
    ...
    minSpendForNegatives:
      toNumber(payload["minSpendForNegatives"]) ?? current.minSpendForNegatives,
    minClicksForNegatives:
      toNumber(payload["minClicksForNegatives"]) ??
      current.minClicksForNegatives,
  };
  ...
  next.minSpendForNegatives = Math.max(
    0,
    Math.min(1000, Math.round(next.minSpendForNegatives)),
  );
  next.minClicksForNegatives = Math.max(
    0,
    Math.min(1000, Math.round(next.minClicksForNegatives)),
  );
