...
import { consumePartnerEmailVerification } from "@/lib/partner-purpose-auth";
...
export async function POST(request: NextRequest): Promise<Response> {
  ...
  try {
    ...
    const rateLimit = await consumeTeamAuthRateLimit({
      action: "partner_email_verification_consume",
      request,
    ...
    const consumed = await consumePartnerEmailVerification({
      rawToken: token,
      request,
      correlationId,
    });
    if (consumed.kind !== "success") {
      return createPartnerPortalV2ErrorResponse(
        "unauthorized",
        401,
        correlationId,
