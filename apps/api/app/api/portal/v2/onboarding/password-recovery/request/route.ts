...
import { requestPartnerPasswordReset } from "@/lib/partner-purpose-auth";
import {
...
export async function POST(request: NextRequest): Promise<Response> {
  ...
  try {
    ...
    const run = await runPortalV2IdempotentMutation({
      principal: `password-recovery:${email}`,
      action: "partner.auth.password_reset.request",
      keyHash: idempotency.keyHash!,
      scope: "POST:/api/portal/v2/onboarding/password-recovery/request",
      payload: { email },
      correlationId,
      execute: async () => {
        await requestPartnerPasswordReset({ email, request, correlationId });
        return {
          status: 202,
          body: {
            ok: true,
            message:
              "If an eligible account exists, a 30-minute reset link will be sent.",
          },
        };
      },
    });
