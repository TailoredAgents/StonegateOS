...
import {
  parsePartnerApplicationResponse,
  respondToPartnerApplication,
...
export async function POST(request: NextRequest): Promise<Response> {
  ...
  try {
    ...
    const payload = parsePartnerApplicationResponse(raw);
    if (!payload) {
      return createPartnerPortalV2ErrorResponse(
        "invalid_fields",
        422,
    ...
    const run = await runPortalV2IdempotentMutation({
      ...
      action: "partner.access_application.respond",
      keyHash: idempotency.keyHash!,
      scope: "POST:/api/portal/v2/onboarding/application/respond",
      payload: { payload, ifMatch: request.headers.get("if-match") },
      correlationId,
      execute: async () => {
        const result = await respondToPartnerApplication({
