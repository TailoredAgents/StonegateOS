...
export async function POST(request: NextRequest, context: RouteContext): Promise<Response> {
  ...
  try {
    ...
    if (!isPortalV2Uuid(requestId)) return createPartnerPortalV2ErrorResponse("not_found", 404, correlationId);
    if (!(await hasAccountJoinRequest(principal.accountId, requestId))) return createPartnerPortalV2ErrorResponse("not_found", 404, correlationId);
    ...
    let raw: unknown;
    try {
      raw = await readBoundedJsonRequest(request, { maximumBytes: 2_048, deadlineMs: 10_000, rejectDuplicateObjectKeys: true });
    } catch (error) {
    ...
    const payload = PartnerJoinDecisionSchema.safeParse(raw);
    ...
    const run = await runPortalV2IdempotentMutation({
      ...
      action: `partner.company_join_request.${payload.data.action}`,
      keyHash: idempotency.keyHash!,
      scope: `POST:/api/portal/v2/join-requests/${requestId}:${principal.accountId}`,
      payload: { decision: payload.data, ifMatch },
      correlationId,
      execute: async () => {
        const rateLimit = await consumeTeamAuthRateLimit({
          action: "partner_join_decision",
          request,
        ...
        return decideAccountJoinRequest({
          principal,
          requestId,
