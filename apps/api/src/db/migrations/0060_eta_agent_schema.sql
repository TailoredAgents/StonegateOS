-- Migration 0055_eta_agent.sql was committed without a matching journal entry.
-- Re-register the complete ETA schema as a forward-only, idempotent migration
-- so both existing databases and clean environments receive every table.
