...
describe("partner account invitation security policy", () => {
  ...
  it("normalizes the mailbox and stores only a deterministic digest", () => {
    expect(normalizeInvitationEmail("  Teammate@Example.COM  ")).toBe(
      "teammate@example.com",
    );
    expect(hashPartnerInvitationToken("one-use-secret")).toMatch(
      /^[0-9a-f]{64}$/u,
    );
    expect(hashPartnerInvitationToken("one-use-secret")).not.toContain(
      "one-use-secret",
    );
  });
  ...
  it("accepts bounded exact create/action/acceptance payloads", () => {
    expect(
      PartnerInvitationCreateSchema.safeParse({
    ...
    expect(
      PartnerInvitationCreateSchema.safeParse({
        ...
        accessLevel: "scoped",
        locationIds: ["33b4c469-5087-4bbd-969d-bb98c8fd3cca"],
        costCenterIds: [],
      }).success,
    ).toBe(true);
    expect(
    ...
    expect(
      PartnerInvitationActionSchema.safeParse({ action: "resend" }).success,
    ).toBe(true);
    expect(
      PartnerInvitationActionSchema.safeParse({ action: "accept" }).success,
    ).toBe(false);
    expect(
      PartnerInvitationAcceptanceSchema.safeParse({ token: "x".repeat(43) })
        .success,
    ).toBe(true);
  });
  ...
  it("prevents an administrator from inviting a role above their authority", () => {
    ...
    expect(
      mayAssignInvitationRole({
        actorCapabilities: administrator,
        roleCapabilities: ["account.read", "payments.initiate"],
      }),
    ).toBe(false);
  });
});
...
describe("partner invitation route and persistence contract", () => {
  const migration = source(
    "apps/api/src/db/migrations/0119_partner_account_invitations.sql",
  );
  const activationMigration = source(
    "apps/api/src/db/migrations/0136_partner_invitation_activation_and_identity_decoupling.sql",
  );
  ...
  const delivery = source(
    "apps/api/src/lib/partner-account-invitation-delivery.ts",
  );
  const collectionRoute = source(
    "apps/api/app/api/portal/v2/invitations/route.ts",
  );
  const itemRoute = source(
    "apps/api/app/api/portal/v2/invitations/[invitationId]/route.ts",
  );
  const acceptRoute = source(
    "apps/api/app/api/portal/v2/invitations/accept/route.ts",
  );
  const purposeAuth = source("apps/api/src/lib/partner-purpose-auth.ts");
  const acceptancePage = source(
    "apps/site/src/app/partners/(public)/invitations/accept/page.tsx",
  );
  const acceptanceCompleteRoute = source(
    "apps/site/src/app/partners/invitations/accept/complete/route.ts",
  );
  ...
  it("is expand-only, tenant-bound, and never persists a raw credential column", () => {
    ...
    expect(activationMigration).toContain("ON DELETE SET NULL");
    expect(activationMigration).toContain(
    ...
    expect(journal.entries).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({
          idx: 133,
          tag: "0136_partner_invitation_activation_and_identity_decoupling",
        }),
      ]),
    );
  });
  ...
  it("requires capability, AAL2 guard, origin, idempotency, rate limits, ETags, and opaque tenant misses", () => {
    ...
    expect(itemRoute.indexOf("await hasPartnerAccountInvitation")).toBeLessThan(
  ...
  it("binds acceptance to account/email/role/scope and queues activation without issuing a session", () => {
    ...
    expect(service).toContain("validateInvitationScopes");
    expect(service).toContain("partnerInvitationLocationScopes");
    expect(service).toContain("partnerMembershipLocationScopes");
    expect(service).toContain('status: "invited"');
    expect(service).toContain("createPartnerActivationChallengeInTransaction");
    expect(service).toContain("activationRequired: true");
    expect(service).not.toContain("tx.insert(partnerSessions)");
    expect(service).not.toContain("tx.insert(contacts)");
    expect(acceptRoute).not.toContain("sessionToken");
    expect(acceptRoute).not.toContain("rememberMe");
    expect(purposeAuth).toContain("applicationId: row.applicationId");
    expect(purposeAuth).toContain("for (const row of rows)");
    expect(purposeAuth).toContain("rows.length > 10");
  });
  ...
  it("keeps duplicate, existing-member, and self invitation delivery responses neutral", () => {
    expect(service).toContain(
      "normalizeInvitationEmail(input.principal.email)",
    );
    expect(service).toContain(
      'action: "partner.account_invitation.request_suppressed"',
    );
    expect(service).toContain(
      'status: 202, body: { ok: true, status: "queued" }',
    );
