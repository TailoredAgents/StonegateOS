import {
  calculatePartnerInvoiceLines,
  PartnerBillingCommandSchema,
} from "@/lib/partner-billing-administration";
import {
  renderPartnerBillingDocument,
  PartnerBillingDocumentSnapshotSchema,
} from "@/lib/partner-billing-document-renderer";
...
describe("CRM-first billing contracts", () => {
  it("calculates fractional quantities in integer minor units with explicit tax and discount", () => {
    expect(
      calculatePartnerInvoiceLines(
        [{ description: "Service", quantity: "1.125", unitAmountCents: 100 }],
        9,
        2,
      ),
    ).toMatchObject({
      subtotalCents: 113,
      totalCents: 120,
      lines: [{ lineTotalCents: 113 }],
    });
    expect(() =>
      calculatePartnerInvoiceLines(
        [{ description: "Service", quantity: "0", unitAmountCents: 100 }],
        0,
        0,
      ),
    ).toThrow();
  ...
  it("requires an explicit separate acknowledgment for an already-given manual refund", () => {
    ...
    expect(
      PartnerBillingCommandSchema.safeParse({
        ...refund,
  ...
  it("renders a real bounded PDF deterministically and rejects internal snapshot metadata", async () => {
    const snapshot = {
      ...
      currency: "USD",
      lines: [{ description: "Service", quantity: "1", amountCents: 12345 }],
      totals: [{ label: "Balance due", amountCents: 12345 }],
      notes: [],
    };
    const first = await renderPartnerBillingDocument(snapshot);
    const second = await renderPartnerBillingDocument(snapshot);
    expect(first.subarray(0, 5).toString()).toBe("%PDF-");
    expect(first.length).toBeGreaterThan(1000);
    expect(first.length).toBeLessThan(100_000);
    expect(first.equals(second)).toBe(true);
    expect(
      PartnerBillingDocumentSnapshotSchema.safeParse({
        ...snapshot,
        internalNotes: "must not be serialized",
      }).success,
    ).toBe(false);
  });
  it("posts a refund with one stable idempotency key and verifies the returned payment binding", async () => {
    ...
    await expect(
      refundSquarePayment(command, {
        accessToken: "local-test-token",
        fetchImpl,
      }),
    ).resolves.toMatchObject({ status: "PENDING" });
    await refundSquarePayment(command, {
      accessToken: "local-test-token",
      fetchImpl,
    });
    ...
    await expect(
      refundSquarePayment(
        { ...command, paymentId: "different-payment" },
        { accessToken: "local-test-token", fetchImpl },
      ),
    ).rejects.toThrow("binding_mismatch");
  });
});

describe("received partner payment admission", () => {
  const input = {
    ...
    reason: "Received at the office",
    confirmation: "PAYMENT ALREADY RECEIVED",
  };
  it("accepts cash/check only with explicit receipt confirmation and whole cents", () => {
    expect(PartnerBillingCommandSchema.safeParse(input).success).toBe(true);
    for (const override of [
      ...
      expect(
        PartnerBillingCommandSchema.safeParse({ ...input, ...override })
          .success,
      ).toBe(false);
  });
});
