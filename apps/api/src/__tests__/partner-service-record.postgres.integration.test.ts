...
async function rollbackTest(
  test: (tx: TeamMutationTransaction) => Promise<void>,
) {
...
async function fixture(tx: TeamMutationTransaction, modelVersion: 1 | 2 = 1) {
...
async function member(
  ...
  await tx.insert(partnerRoleTemplates).values({
    id: roleId,
    partnerAccountId: accountId,
    key: `test_${roleId.replaceAll("-", "")}`,
    name: "Local test role",
    description: "Test only",
    capabilities: [capability, "jobs.read"],
  });
...
suite("partner service record / real PostgreSQL", () => {
  ...
  it("deduplicates committed notifications and excludes non-billing and ungranted scoped people", async () =>
    rollbackTest(async (tx) => {
      const f = await fixture(tx);
      const billing = await member(tx, f.accountId, "invoices.read"),
        operations = await member(tx, f.accountId, "messages.read"),
        scoped = await member(tx, f.accountId, "invoices.read", true);
      for (const membershipId of [billing, billing, operations, scoped])
      ...
      const notices = await tx
        ...
        .where(eq(partnerNotifications.partnerAccountId, f.accountId));
      expect(notices).toHaveLength(1);
      expect(notices[0]!.membershipId).toBe(billing);
      const deliveries = await tx
        .select()
        .from(partnerNotificationDeliveries)
        .where(eq(partnerNotificationDeliveries.partnerAccountId, f.accountId));
      expect(deliveries).toHaveLength(9);
      expect(
        deliveries
          .filter((row) => row.membershipId !== billing)
          .every((row) => row.state === "suppressed"),
      ).toBe(true);
      expect(
        deliveries.find(
          (row) => row.membershipId === billing && row.channel === "sms",
        )!.state,
      ).toBe("suppressed");
    }));
  it("does not let unscanned PDF documents satisfy partner completion", async () =>
    rollbackTest(async (tx) => {
      const f = await fixture(tx),
        assetId = randomUUID();
      ...
      await tx.insert(mediaAssets).values({
        ...
        originalObjectKey: `quarantine/${assetId}`,
        status: "ready",
      ...
      expect(
        (await evaluatePartnerProofCompletion(tx, f.appointmentId)).kind,
      ).toBe("missing");
      await tx
        .update(mediaAssets)
        .set({ sourceMetadata: { scanStatus: "clean" } })
        .where(eq(mediaAssets.id, assetId));
      expect(
        (await evaluatePartnerProofCompletion(tx, f.appointmentId)).kind,
      ).toBe("satisfied");
    }));
  it("queues completion records only for explicitly associated partner jobs", async () =>
    rollbackTest(async (tx) => {
      const f = await fixture(tx);
      ...
      expect((await evaluatePartnerProofCompletion(tx, normalId)).kind).toBe(
        "not_partner_job",
      );
      ...
      const records = await tx
        .select()
        .from(outboxEvents)
        .where(eq(outboxEvents.type, "partner.proof.prepare"));
      expect(
        records.filter(
          (row) =>
            row.payload &&
            typeof row.payload === "object" &&
            row.payload["jobId"] === f.jobId,
        ),
      ).toHaveLength(1);
  ...
  it.each([1, 2] as const)(
    "keeps one explicit model %i job thread, retries a staff reply once, and excludes internal notes",
    async (modelVersion) =>
      rollbackTest(async (tx) => {
        const f = await fixture(tx, modelVersion),
          requester = await member(tx, f.accountId, "messages.read"),
          staffId = randomUUID();
        await tx
          .update(partnerBookings)
          .set({ requestedByMembershipId: requester })
          .where(eq(partnerBookings.id, f.jobId));
        await tx
          .insert(teamMembers)
          .values({ id: staffId, name: "Local staff" });
        const thread = await ensurePartnerJobThread(tx, f.accountId, f.jobId);
        expect(
          (await ensurePartnerJobThread(tx, f.accountId, f.jobId)).id,
        ).toBe(thread.id);
        expect(
          await tx
            .select()
            .from(conversationThreads)
            .where(eq(conversationThreads.partnerBookingId, f.jobId)),
        ).toHaveLength(1);
        ...
        expect(people[0]!.partnerMembershipId).toBe(requester);
        ...
        const reply = {
          ...
          body: "Your requested work is confirmed.",
          attachmentIds: [],
          mutation,
        };
        const first = await sendStaffPartnerJobMessageInTransaction(tx, reply);
        expect(
          (await sendStaffPartnerJobMessageInTransaction(tx, reply)).id,
        ).toBe(first.id);
        await expect(
        ...
        await sendStaffPartnerJobMessageInTransaction(tx, {
          ...
          audience: "internal",
          body: "Internal only",
          mutation: { ...mutation, idempotencyKeyHash: "b".repeat(64) },
        });
        ...
        expect(visible[0]!.body).toBe(reply.body);
        ...
        expect(notifications[0]!.membershipId).toBe(requester);
      }),
  );
});
