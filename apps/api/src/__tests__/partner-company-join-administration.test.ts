...
describe("partner verified-domain join administration", () => {
  it("revalidates an exact non-public work domain", () => {
    expect(verifiedJoinDomain("https://www.example.com/about", "person@example.com")).toBe(true);
    expect(verifiedJoinDomain("example.com", "person@other.example.com")).toBe(false);
  ...
  it("accepts bounded exact approve, decline, and needs-information decisions", () => {
    ...
    expect(PartnerJoinDecisionSchema.safeParse({ action: "decline", note: "Domain could not be verified." }).success).toBe(true);
    expect(PartnerJoinDecisionSchema.safeParse({ action: "decline", note: "" }).success).toBe(false);
    expect(PartnerJoinDecisionSchema.safeParse({ action: "needs_information", note: "Confirm your relationship to the company." }).success).toBe(true);
    expect(PartnerJoinDecisionSchema.safeParse({ action: "needs_information", note: "x", internalAccountId: "override" }).success).toBe(false);
  });
...
describe("partner join-decision route contract", () => {
  const service = source("apps/api/src/lib/partner-company-join-administration.ts");
  const collection = source("apps/api/app/api/portal/v2/join-requests/route.ts");
  const item = source("apps/api/app/api/portal/v2/join-requests/[requestId]/route.ts");
  const manager = source("apps/site/src/app/partners/components/PartnerJoinRequestManager.tsx");

  it("uses capability/AAL2, origin, idempotency, rate, revision, and tenant guards", () => {
    ...
    expect(item.indexOf("await hasAccountJoinRequest")).toBeLessThan(item.lastIndexOf("readPortalV2IdempotencyKey"));
  });

  it("forbids self approval, revalidates domain and role authority, and never reactivates stale access", () => {
    expect(service).toContain("request.partnerUserId === input.principal.partnerUserId");
    expect(service).toContain('reason: "self_approval"');
    expect(service).toContain("verifiedJoinDomain(account.domain, request.userEmail)");
    expect(service).toContain("mayAssignInvitationRole");
    expect(service).toContain('reason: existingMembership.status === "active" ? "already_member" : "existing_access_requires_staff_review"');
  ...
  it("scopes every resource lookup and mutation to the selected account with opaque misses", () => {
    expect(service).toContain("eq(partnerCompanyJoinRequests.partnerAccountId, accountId)");
    expect(service).toContain("eq(partnerAccountMemberships.partnerAccountId, accountId)");
    expect(service).toContain('{ status: 404, body: { ok: false, error: "not_found" } }');
    expect(service).toContain('.for("update")');
  });

  it("atomically queues one preference-aware in-app and email effect per decision", () => {
    expect(service).toContain("partnerNotificationPreferences.eventKey, \"account_access\"");
    expect(service).toContain("partnerNotifications");
    expect(service).toContain("deterministicNotificationId(operationHash)");
    expect(service).toContain(".onConflictDoNothing({ target: partnerNotifications.id })");
    expect(service).toContain("arePartnerPortalOutboundNotificationsEnabled(input.decisionAccountId)");
    expect(service).toContain("queueSystemOutboundMessage");
    expect(service).toContain("nextQuietHoursEnd");
    expect(service).toContain("dedupeKey: `partner.join.decision:${operationHash}`");
    expect(service).toContain('kind: "partner.company_join_decision"');
    expect(service).toContain("defaultNotificationTarget(tx, request.partnerUserId)");
  });
  ...
  it("supports needs-information decisions without exposing reviewer notes in delivery copy", () => {
    ...
    expect(service).not.toContain("reviewNote: input.decision.note,\n    emailBody");
