...
import { loadPartnerInvitationIssuer } from "@/lib/partner-invitation-authority";
...
function safeDeliveryUrl(
  ...
  try {
    ...
    if (!configuredBase || url.origin !== new URL(configuredBase).origin)
      return null;
...
export async function processPartnerAccountInvitationEmail(input: {
  ...
  const prepared = await db.transaction(async (tx) => {
    const [account] = await tx
      ...
      .for("update")
      .limit(1);
    if (!account || !account.enabled || account.lifecycle !== "active")
    ...
    const [row] = await tx
      .select({
        ...
        invitedByMembershipId: partnerAccountInvitations.invitedByMembershipId,
        invitedByTeamMemberId: partnerAccountInvitations.invitedByTeamMemberId,
        roleTemplateId: partnerAccountInvitations.roleTemplateId,
    ...
    const now = new Date();
    const [role] = await tx
      .select({ capabilities: partnerRoleTemplates.capabilities })
    ...
    if (
      ...
      !(await loadPartnerInvitationIssuer(tx, {
        ...
        membershipId: row.invitedByMembershipId,
        teamMemberId: row.invitedByTeamMemberId,
      ...
      await tx
        ...
        .set({
          ...
          deliveryDetail: "invitation_authority_changed",
          updatedAt: now,
        })
    ...
    const [restrictedRecipient] = await tx
    ...
    return claimed
      ? {
          ...
          row: { ...row, accountName: account.name },
        }
  ...
  const body = [
    ...
    "",
    "Open this one-use invitation within seven days to set or confirm your password:",
    parsed.url,
    "",
    "If you weren’t expecting this invitation, you can ignore this email.",
    "Need help? Email sales@stonegatejunkremoval.com or call 404-777-2631.",
  ].join("\n");
