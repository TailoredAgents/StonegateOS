...
export async function requestQuoteChangesAction(formData: FormData) {
  ...
  await fetch(`${API_BASE_URL.replace(/\/$/, "")}/api/public/quotes/${token}/changes`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
      reason: typeof reason === "string" && reason.trim().length > 0 ? reason.trim() : "Other",
...
export async function bookQuoteAction(formData: FormData) {
  ...
  const bookResponse = await fetch(`${API_BASE_URL.replace(/\/$/, "")}/api/public/quotes/${token}/book`, {
    ...
    body: JSON.stringify({
      startAt,
      holdId: hold.holdId,
      customerNote: typeof customerNote === "string" && customerNote.trim().length > 0 ? customerNote.trim() : undefined
    })
  });
  if (!bookResponse.ok) {
    redirect(`/quote/${token}?booking=failed`);
...
export default async function PublicQuotePage({
  ...
  const approvalFlag = typeof query["approval"] === "string" ? query["approval"] : null;
  const changesFlag = typeof query["changes"] === "string" ? query["changes"] : null;
  const quote = await fetchQuote(token, preview);
  if (!quote) notFound();

  const availability =
    (quote.status === "sent" || quote.status === "accepted") && !quote.acceptedAppointmentId && !quote.expired
      ? await fetchAvailability(token)
      : null;
  const showApproveAndBook = (quote.status === "sent" || quote.status === "accepted") && !quote.acceptedAppointmentId && !quote.expired;
  const showRefreshForm = quote.expired || quote.displayStatus === "refresh_requested";
  const company = getPublicCompanyProfile();
  const hasAvailableSlots = (availability?.days ?? []).some((day) => day.slots.length > 0);
    ...
    <main className="min-h-screen bg-[#f6f4ef] text-neutral-950">
      <section className="border-b border-neutral-200 bg-white">
        <div className="mx-auto flex max-w-6xl flex-col gap-8 px-4 py-8 sm:px-6 lg:px-8">
          <header className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
            <div className="max-w-3xl">
              ...
              <p className="mt-3 text-base leading-7 text-neutral-600">
                Quote {quote.quoteNumber} prepared for {quote.customerName}. Review the scope, approve the quote, and book your service window in one step.
              </p>
            </div>
            <div className="flex flex-col gap-3 rounded-2xl border border-neutral-200 bg-neutral-50 p-4 text-sm shadow-sm lg:min-w-72">
              <div className="flex items-center justify-between gap-3">
                ...
                <span className={`rounded-full border px-3 py-1 text-xs font-semibold ${statusTone(quote.displayStatus)}`}>
                  {statusLabel(quote.displayStatus)}
                </span>
              </div>
              <div>
                <div className="text-xs font-semibold uppercase tracking-[0.14em] text-neutral-500">Valid until</div>
                <div className={`mt-1 text-lg font-semibold ${quote.expired ? "text-rose-700" : "text-primary-950"}`}>
          ...
          <div className="grid gap-4 md:grid-cols-3">
            <a href={`tel:${company.phoneE164}`} className="rounded-xl border border-neutral-200 bg-white px-4 py-3 text-sm font-semibold text-primary-800 shadow-sm hover:border-primary-300">
              Call {company.phoneDisplay}
            </a>
            <a href={smsHref} className="rounded-xl border border-neutral-200 bg-white px-4 py-3 text-sm font-semibold text-primary-800 shadow-sm hover:border-primary-300">
              Text {company.phoneDisplay}
            </a>
            <a href={mailHref} className="rounded-xl border border-neutral-200 bg-white px-4 py-3 text-sm font-semibold text-primary-800 shadow-sm hover:border-primary-300">
              Email {company.email}
            </a>
          </div>
        </div>
      ...
      <div className="mx-auto grid max-w-6xl gap-6 px-4 py-8 sm:px-6 lg:grid-cols-[minmax(0,1fr)_360px] lg:px-8">
        <div className="space-y-6">
          ...
          {changesFlag === "sent" ? (
            <section className="rounded-2xl border border-amber-300 bg-amber-50 p-4 text-sm font-medium text-amber-900">
              Change request received. Your quote is still available to approve while Stonegate reviews your request.
            </section>
          ) : null}
          ...
          <section className="rounded-3xl border border-neutral-200 bg-white p-6 shadow-sm sm:p-8">
            <div className="flex flex-col gap-6 sm:flex-row sm:items-start sm:justify-between">
              <div>
                <p className="text-xs font-semibold uppercase tracking-[0.14em] text-neutral-500">Proposal total</p>
                <div className="mt-2 text-5xl font-semibold tracking-tight text-primary-950">{formatCurrency(quote.total)}</div>
                <p className="mt-2 text-sm text-neutral-600">{paymentTerms(quote)}</p>
              </div>
              <div className="rounded-2xl border border-neutral-200 bg-neutral-50 p-4 text-sm text-neutral-700 sm:min-w-64">
                <div className="font-semibold text-primary-950">Service property</div>
                <div className="mt-2">{[quote.addressLine1, quote.serviceArea].filter(Boolean).join(", ")}</div>
                <div className="mt-4 font-semibold text-primary-950">Estimated duration</div>
                <div className="mt-1">{Math.round((quote.jobDurationMinutes / 60) * 10) / 10} hr</div>
              </div>
            </div>
          </section>
          ...
          <section className="rounded-3xl border border-neutral-200 bg-white p-6 shadow-sm sm:p-8">
            <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
              <div>
                <p className="text-xs font-semibold uppercase tracking-[0.14em] text-primary-700">Scope</p>
                <h2 className="mt-1 text-2xl font-semibold text-primary-950">What this quote includes</h2>
              </div>
            </div>
            ...
            <div className="mt-6 grid gap-3 sm:grid-cols-3">
              <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-950">
                ...
                <p className="mt-1 text-emerald-900">Line items and total are visible before you approve.</p>
              </div>
              <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-950">
                <div className="font-semibold">Disposal included</div>
                <p className="mt-1 text-emerald-900">The quoted service includes haul-away and disposal for the listed scope.</p>
              </div>
              <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-950">
                <div className="font-semibold">Make-It-Right Guarantee</div>
                <p className="mt-1 text-emerald-900">If something is not right, Stonegate will work to make it right.</p>
              </div>
            </div>
          </section>

          <section className="rounded-3xl border border-neutral-200 bg-white p-6 shadow-sm sm:p-8">
            <p className="text-xs font-semibold uppercase tracking-[0.14em] text-primary-700">Pricing</p>
            <h2 className="mt-1 text-2xl font-semibold text-primary-950">Line-item quote</h2>
                  ...
                  {quote.lineItems.map((item) => (
                    <tr key={item.id}>
                      <td className="px-4 py-4">
                        <div className="font-semibold text-primary-950">{item.label}</div>
                        {item.category ? <div className="text-xs uppercase tracking-[0.12em] text-neutral-500">{item.category}</div> : null}
                      </td>
                      <td className="whitespace-nowrap px-4 py-4 text-right font-semibold">{formatCurrency(item.amount)}</td>
                    </tr>
                  ))}
                  <tr className="bg-neutral-50">
                    <td className="px-4 py-4 font-semibold text-primary-950">Subtotal</td>
                    <td className="px-4 py-4 text-right font-semibold text-primary-950">{formatCurrency(quote.subtotal)}</td>
                  </tr>
                  <tr className="bg-primary-950 text-white">
                    <td className="px-4 py-4 text-base font-semibold">Total</td>
                    <td className="px-4 py-4 text-right text-base font-semibold">{formatCurrency(quote.total)}</td>
                  </tr>
                </tbody>
              </table>
            </div>
          ...
          <section className="rounded-3xl border border-neutral-200 bg-white p-6 shadow-sm sm:p-8">
            <p className="text-xs font-semibold uppercase tracking-[0.14em] text-primary-700">Next steps</p>
            <h2 className="mt-1 text-2xl font-semibold text-primary-950">What happens after approval</h2>
            <div className="mt-5 grid gap-4 sm:grid-cols-4">
              {["Approve quote", "Pick a time", "Crew confirms", "Service completed"].map((step, index) => (
                <div key={step} className="rounded-2xl border border-neutral-200 bg-neutral-50 p-4 text-sm">
                  <div className="flex h-8 w-8 items-center justify-center rounded-full bg-primary-950 text-sm font-semibold text-white">{index + 1}</div>
                  <div className="mt-3 font-semibold text-primary-950">{step}</div>
                </div>
              ))}
            </div>
          </section>
          ...
          <section className="rounded-3xl border border-neutral-200 bg-white p-6 shadow-sm sm:p-8">
            <details>
              <summary className="cursor-pointer list-none text-lg font-semibold text-primary-950">Request changes to this quote</summary>
              <p className="mt-2 text-sm text-neutral-600">Send a structured request to Stonegate. The quote stays available to approve while the team reviews it.</p>
              <form action={requestQuoteChangesAction} className="mt-5 space-y-4">
                <input type="hidden" name="token" value={token} />
                <label className="block text-sm font-semibold text-neutral-700">
                  What needs to change?
                  <select name="reason" className="mt-2 w-full rounded-xl border border-neutral-300 bg-white px-3 py-3 text-sm text-neutral-900">
                ...
                <label className="block text-sm font-semibold text-neutral-700">
                  Details
                  <textarea name="message" rows={4} className="mt-2 w-full rounded-xl border border-neutral-300 bg-white px-3 py-3 text-sm text-neutral-900" placeholder="Tell us what should change." />
                </label>
          ...
          <section className="rounded-3xl border border-neutral-200 bg-white p-6 text-sm text-neutral-700 shadow-sm sm:p-8">
            <h2 className="text-lg font-semibold text-primary-950">Terms and assumptions</h2>
        ...
        <aside className="space-y-6 lg:sticky lg:top-6 lg:self-start">
          <section className="rounded-3xl border border-primary-200 bg-white p-6 shadow-lg shadow-neutral-200/70">
            <p className="text-xs font-semibold uppercase tracking-[0.14em] text-primary-700">Approve and schedule</p>
            <h2 className="mt-2 text-2xl font-semibold text-primary-950">Ready to move forward?</h2>
            {showRefreshForm ? (
              <div className="mt-5 rounded-2xl border border-rose-200 bg-rose-50 p-4 text-sm text-rose-800">
                {quote.refreshRequestedAt ? (
                  ...
                  <form action={refreshQuoteAction} className="space-y-3">
                    ...
                    <input type="hidden" name="token" value={token} />
                    <button className="w-full rounded-xl border border-rose-300 bg-white px-4 py-3 text-sm font-semibold text-rose-700">Request refresh</button>
                  </form>
                )}
              </div>
            ) : showApproveAndBook ? (
                ...
                {hasAvailableSlots ? (
                  <form action={bookQuoteAction} className="space-y-4">
                    ...
                    <label className="block text-sm font-semibold text-neutral-700">
                      ...
                      <textarea name="customerNote" rows={3} className="mt-2 w-full rounded-xl border border-neutral-300 bg-white px-3 py-3 text-sm text-neutral-900" placeholder="Gate code, access notes, timing preference, or anything we should know." />
                    </label>
                    {availability?.days?.map((day) =>
                      day.slots.length ? (
                        <div key={day.date}>
                          <div className="text-sm font-semibold text-neutral-700">{formatDay(day.date)}</div>
                          <div className="mt-2 grid grid-cols-2 gap-2">
                            {day.slots.slice(0, 4).map((slot) => (
                              ...
                              >
                                {quote.status === "sent" ? "Approve and book" : "Book"} {slot.label}
                              </button>
                            ))}
                          </div>
                        </div>
                  ...
                  <form action={acceptQuoteAction} className="space-y-3">
                    ...
                    <p className="rounded-2xl border border-dashed border-neutral-300 bg-neutral-50 p-4 text-sm text-neutral-600">
                      No online windows are available right now. Approve the quote and Stonegate will schedule with you directly.
                    </p>
          ...
          <section className="rounded-3xl border border-neutral-200 bg-white p-6 shadow-sm">
            <h2 className="text-lg font-semibold text-primary-950">Need help?</h2>
            <div className="mt-4 space-y-2 text-sm">
              <a href={`tel:${company.phoneE164}`} className="block rounded-xl border border-neutral-200 px-3 py-3 font-semibold text-primary-800 hover:border-primary-300">Call {company.phoneDisplay}</a>
              <a href={smsHref} className="block rounded-xl border border-neutral-200 px-3 py-3 font-semibold text-primary-800 hover:border-primary-300">Text {company.phoneDisplay}</a>
              <a href={mailHref} className="block rounded-xl border border-neutral-200 px-3 py-3 font-semibold text-primary-800 hover:border-primary-300">Email {company.email}</a>
            </div>
          </section>

          <section className="rounded-3xl border border-neutral-200 bg-white p-6 shadow-sm">
            <details>
              <summary className="cursor-pointer list-none text-sm font-semibold text-neutral-700">Decline quote</summary>
              <form action={declineQuoteAction} className="mt-4 space-y-3">
                <input type="hidden" name="token" value={token} />
                <select name="reason" className="w-full rounded-xl border border-neutral-300 bg-white px-3 py-3 text-sm text-neutral-900">
                ...
                <textarea name="notes" rows={3} className="w-full rounded-xl border border-neutral-300 bg-white px-3 py-3 text-sm text-neutral-900" placeholder="Optional note" />
                <button className="w-full rounded-xl border border-rose-300 bg-rose-50 px-4 py-3 text-sm font-semibold text-rose-700 hover:bg-rose-100">Send rejection</button>
              </form>
            </details>
          </section>
        </aside>
      ...
      <footer className="border-t border-neutral-200 bg-white px-4 py-6 text-xs text-neutral-500 sm:px-6">
        <div className="mx-auto flex max-w-6xl flex-wrap items-center justify-between gap-3">
          <span>{company.name} | Licensed and insured | Make-It-Right Guarantee</span>
          <Link href="/" className="font-semibold text-accent-700 hover:underline">
            Back to homepage
          </Link>
        </div>
      </footer>
