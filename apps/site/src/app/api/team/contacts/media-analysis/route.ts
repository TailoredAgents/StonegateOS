...
export async function GET(request: Request): Promise<Response> {
  ...
  const upstream = await callAdminApi(
    `/api/admin/contacts/${encodeURIComponent(contactId)}/media-analysis${includeQuotePrice ? "?includeQuotePrice=1" : ""}`,
    {
...
export async function POST(request: Request): Promise<Response> {
  ...
  const includeQuotePrice = url.searchParams.get("includeQuotePrice") === "1";
  const upstream = await callAdminApi(
    `/api/admin/contacts/${encodeURIComponent(contactId)}/media-analysis/rebuild${includeQuotePrice ? "?includeQuotePrice=1" : ""}`,
    {
      method: "POST",
      headers: { Accept: "application/json" },
    },
