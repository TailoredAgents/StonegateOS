...
export async function POST(
  ...
  const parsed = buildExpenseReasonBody(form);
  if (!parsed.ok) {
  ...
  try {
    const apiResponse = await callAdminApiAs(
      auth.principal,
      `/api/admin/expenses/${encodeURIComponent(expenseId)}/void`,
      {
        method: "POST",
        headers: {
    ...
    const outcome = await readExpenseMutationResponse(
      apiResponse,
      "Expense voided with a linked reversal.",
    ...
    return expenseFlashRedirect(
      redirectTo,
      "The expense service could not be reached. The expense was not reported as voided.",
      false,
    );
  }
}
