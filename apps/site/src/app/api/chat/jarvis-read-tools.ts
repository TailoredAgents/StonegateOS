...
type AdminContext = {
  callApi?: (path: string, init?: RequestInit) => Promise<Response>;
};
...
function toolSummaryLines(result: JarvisReadToolResult): string[] {
  ...
  if (result.tool === "google.ads.summary") {
    const totals = isRecord(data["totals"]) ? data["totals"] : null;
    if (!totals) return [];
    const cost = typeof totals["cost"] === "string" ? totals["cost"] : "0";
    ...
    const conversions =
      typeof conversionsRaw === "string"
        ? Number(conversionsRaw)
        : asNumber(conversionsRaw, 0);
  ...
  if (result.tool === "web.analytics.errors") {
    ...
    const total = items.reduce<number>(
      (acc, item) => acc + (isRecord(item) ? asNumber(item["count"], 0) : 0),
      0,
    );
    const top = items
      .slice(0, 3)
      .map((item) =>
        isRecord(item)
          ? `${asString(item["event"]) ?? "event"}: ${asNumber(item["count"], 0)}`
          : null,
      )
      .filter((v): v is string => Boolean(v && v.trim().length));
    return [
      `Tracked failures: ${total}${top.length ? ` (top: ${top.join(", ")})` : ""}`,
    ];
  }

  if (result.tool === "schedule.summary") {
    const total = asNumber(data["total"], 0);
    const byStatus = isRecord(data["byStatus"]) ? data["byStatus"] : null;
    const statusBits = byStatus
      ...
      : "";
    return [
      `Schedule: ${total} appt(s)${statusBits ? ` (${statusBits})` : ""}`,
    ];
  }
  ...
  if (result.tool === "finance.revenue.summary") {
    ...
    const windows = isRecord(data["windows"]) ? data["windows"] : null;
    if (!windows) return [];
    const lines: string[] = [];
    for (const key of ["monthToDate", "last30Days", "yearToDate"] as const) {
      const w = isRecord(windows[key]) ? windows[key] : null;
      if (!w) continue;
      ...
      const label =
        key === "monthToDate"
          ...
          : key === "last30Days"
            ...
            : "YTD";
      lines.push(
        `Revenue ${label}: ${fmtMoneyCents(totalCents, currency)} (${count} completed)`,
      );
    }
    return lines;
  }
      ...
      const label =
        key === "monthToDate"
          ...
          : key === "last30Days"
            ...
            : "YTD";
      lines.push(
        `Expenses ${label}: ${fmtMoneyCents(totalCents, currency)} (${count} items)`,
      );
    }
    return lines;
  }
  ...
  if (result.tool === "finance.pnl") {
    const revenue = isRecord(data["revenue"]) ? data["revenue"] : null;
    const expenses = isRecord(data["expenses"]) ? data["expenses"] : null;
    const currency =
      (revenue && typeof revenue["currency"] === "string"
        ? revenue["currency"]
        : null) ?? "USD";
    const rw =
      revenue && isRecord(revenue["windows"]) ? revenue["windows"] : null;
    const ew =
      expenses && isRecord(expenses["windows"]) ? expenses["windows"] : null;
    if (!rw || !ew) return [];

    const lines: string[] = [];
    for (const key of ["monthToDate", "last30Days", "yearToDate"] as const) {
      const r = isRecord(rw[key]) ? rw[key] : null;
      const e = isRecord(ew[key]) ? ew[key] : null;
      if (!r || !e) continue;
      const profit =
        asNumber(r["totalCents"], 0) - asNumber(e["totalCents"], 0);
      const label =
        key === "monthToDate"
          ? "MTD"
          : key === "last30Days"
  ...
  if (result.tool === "google.ads.analyst.status") {
    const latest = isRecord(data["latest"]) ? data["latest"] : null;
    const createdAt =
      latest && typeof latest["createdAt"] === "string"
        ? latest["createdAt"]
        : "";
    const rangeDays =
      latest && typeof latest["rangeDays"] === "number"
        ? String(latest["rangeDays"])
        : "";
    const health = isRecord(data["health"]) ? data["health"] : null;
    const lastSuccessAt =
      health && typeof health["lastSuccessAt"] === "string"
        ? health["lastSuccessAt"]
        : "";
    const lastFailureAt =
      health && typeof health["lastFailureAt"] === "string"
  ...
  if (result.tool === "google.ads.analyst.recommendations") {
    ...
    for (const item of items) {
      ...
      const term =
        payload && typeof payload["term"] === "string"
          ? payload["term"]
          : payload && typeof payload["keyword"] === "string"
            ? payload["keyword"]
            : null;
      if (kind === "negative_keyword" && term && topTerms.length < 5)
        topTerms.push(String(term));
    }
    ...
    return [
      ...
      ...(topTerms.length
        ? [`Top negative keywords: ${topTerms.join(", ")}`]
        : []),
    ];
  }
  ...
  if (result.tool === "crm.pipeline") {
    const lanes = Array.isArray(data["lanes"])
      ? (data["lanes"] as unknown[])
      : [];
    const summaryCounts = isRecord(data["stageCounts"])
      ? data["stageCounts"]
      : null;
    const stageCounts = summaryCounts
      ? Object.entries(summaryCounts).map(([stage, count]) => ({
          stage,
      ...
      : lanes
          .map((lane) => {
            ...
            const stage = asString(lane["stage"]) ?? "";
            const contacts = Array.isArray(lane["contacts"])
              ? (lane["contacts"] as unknown[])
              : [];
    ...
    const bits = stageCounts
      .slice(0, 6)
      .map((v) => `${v.stage}: ${v.count}`)
      .join(", ");
  ...
  if (result.tool === "inbox.threads.list") {
    ...
    const top = threads
      ...
      .map((t) => {
        ...
        const channel = asString(t["channel"]) ?? "unknown";
        const status = asString(t["status"]) ?? "unknown";
        const subject = typeof t["subject"] === "string" ? t["subject"] : "";
        const preview =
          typeof t["lastMessagePreview"] === "string"
      ...
      .filter((v): v is string => Boolean(v && v.trim().length));
    return [
      `Inbox threads: ${threads.length} match(es).`,
      ...(top.length ? top.map((l) => `- ${l}`) : []),
    ];
  }
  ...
  if (result.tool === "crm.contact.snapshot") {
    const contacts = Array.isArray(data["contacts"])
      ...
      : [];
    const first =
      contacts.length > 0 && isRecord(contacts[0]) ? contacts[0] : null;
    if (!first) return [];
    ...
    const phone =
      typeof first["phoneE164"] === "string"
        ...
        : typeof first["phone"] === "string"
          ? first["phone"]
          : "";
    const pipeline = isRecord(first["pipeline"]) ? first["pipeline"] : null;
    const stage =
      pipeline && typeof pipeline["stage"] === "string"
        ? pipeline["stage"]
        : "";
    const props = Array.isArray(first["properties"])
      ? (first["properties"] as unknown[])
      : [];
    const prop = props.length > 0 && isRecord(props[0]) ? props[0] : null;
    const city = prop && typeof prop["city"] === "string" ? prop["city"] : "";
    const zip =
      prop && typeof prop["postalCode"] === "string" ? prop["postalCode"] : "";
    const lastActivityAt =
      typeof first["lastActivityAt"] === "string"
  ...
  if (result.tool === "crm.contact.instant_quote_photos") {
    ...
    const photoUrls = Array.isArray(data["photoUrls"])
      ? (data["photoUrls"] as unknown[])
      : [];
    const first = quotes.length > 0 && isRecord(quotes[0]) ? quotes[0] : null;
    const createdAt =
      first && typeof first["createdAt"] === "string" ? first["createdAt"] : "";
    const jobTypes =
      first && Array.isArray(first["jobTypes"])
        ? (first["jobTypes"] as unknown[])
        : [];
    const perceivedSize =
      first && typeof first["perceivedSize"] === "string"
        ? first["perceivedSize"]
        : "";
    const jobTypesText = jobTypes
      .filter((j) => typeof j === "string" && j.trim().length)
      .slice(0, 4)
      .join(", ");
    return [
  ...
  if (result.tool === "crm.contact.omni_facts") {
    const facts = isRecord(data["facts"]) ? data["facts"] : null;
    if (!facts) return [];
    const pipelineStage =
      typeof facts["pipelineStage"] === "string" ? facts["pipelineStage"] : "";
    const knownZip =
      typeof facts["knownZip"] === "string" ? facts["knownZip"] : "";
    const missingFields = Array.isArray(facts["missingFields"])
      ? (facts["missingFields"] as unknown[])
      : [];
    const missing = missingFields
      .filter((v): v is string => typeof v === "string" && v.trim().length > 0)
      .slice(0, 4);

    const instantQuote = isRecord(facts["instantQuote"])
      ? facts["instantQuote"]
      : null;
    const photoUrls =
      instantQuote && Array.isArray(instantQuote["photoUrls"])
        ? (instantQuote["photoUrls"] as unknown[])
        : [];
    const jobTypes =
      instantQuote && Array.isArray(instantQuote["jobTypes"])
        ? (instantQuote["jobTypes"] as unknown[])
        : [];
    const size =
      instantQuote && typeof instantQuote["perceivedSize"] === "string"
        ? instantQuote["perceivedSize"]
        : "";
    const typesText = jobTypes
      .filter((j) => typeof j === "string" && j.trim().length)
      .slice(0, 4)
      .join(", ");
    const photoCount = photoUrls.filter(
      (u) => typeof u === "string" && u.trim().length,
    ).length;

    const otherThreads = Array.isArray(facts["otherChannelThreads"])
      ? (facts["otherChannelThreads"] as unknown[])
      : [];
    const threadCount = otherThreads.length;

    const lines: string[] = [];
    lines.push(
      `Omni facts: ${pipelineStage ? `pipeline ${pipelineStage}` : "pipeline unknown"}${knownZip ? `, zip ${knownZip}` : ""}`,
    );
    if (instantQuote) {
      lines.push(
        `Instant quote: ${photoCount} photo(s)${size ? `, size ${size}` : ""}${typesText ? `, types ${typesText}` : ""}`,
      );
    }
  ...
  if (result.tool === "crm.contact.sales_agent_memory") {
    ...
    const liveContext = isRecord(data["liveContext"])
      ? data["liveContext"]
      : null;
    if (!memory) return [];

    const summary =
      typeof memory["summary"] === "string" ? memory["summary"] : "";
    const customerIntent =
      typeof memory["customerIntent"] === "string"
        ? memory["customerIntent"]
        : "";
    const channelPreference =
      typeof memory["channelPreference"] === "string"
        ? memory["channelPreference"]
        : "";
    const bookingReadiness =
      typeof memory["bookingReadiness"] === "string"
        ? memory["bookingReadiness"]
        : "";
    const quoteConfidence =
      typeof memory["quoteConfidence"] === "string"
        ? memory["quoteConfidence"]
        : "";
    const missingFields = Array.isArray(memory["missingFields"])
      ? (memory["missingFields"] as unknown[])
      : [];
    const objections = Array.isArray(memory["objections"])
      ? (memory["objections"] as unknown[])
      : [];
    const nextAppointment =
      liveContext && isRecord(liveContext["nextAppointment"])
        ? liveContext["nextAppointment"]
        : null;
    ...
    const lines: string[] = [];
    if (summary)
      lines.push(`Sales agent memory: ${truncateText(summary, 280)}`);
    if (
      customerIntent ||
      channelPreference ||
      bookingReadiness ||
    ...
    if (nextAppointmentType)
      lines.push(`Next appointment type: ${nextAppointmentType}`);
    const missing = missingFields
      .filter((v): v is string => typeof v === "string" && v.trim().length > 0)
      .slice(0, 5);
    ...
    if (objectionList.length)
      lines.push(`Objections: ${objectionList.join(", ")}`);
    return lines;
  }
...
> {
  if (!ctx.callApi) {
    return { ok: false, status: 428, error: "verified_principal_missing" };
  }
  ...
  const path = `${input.path}${query.size ? `?${query.toString()}` : ""}`;
  const res = await ctx
    .callApi(path, {
...
function toolNotConfigured(
  tool: JarvisReadToolName,
  error: string,
): JarvisReadToolResult {
...
export async function runJarvisReadTool(
  ctx: AdminContext,
  call: JarvisReadToolCall,
): Promise<JarvisReadToolResult> {
  if (!ctx.callApi) {
    return toolNotConfigured(
      call.tool,
      "Verified team principal is unavailable",
    );
  }
  ...
  switch (tool) {
    case "policy.get_all": {
      ...
      if (!res.ok)
        return res.status === 428
          ? toolNotConfigured(tool, res.error)
          : toolUnavailable(tool, res.status, res.error, res.data);
    ...
    case "web.analytics.vitals": {
      ...
      const utmCampaign =
        tool === "web.analytics.summary" ? asString(args["utmCampaign"]) : null;
      const res = await adminFetchJson(ctx, {
        path,
        query: { rangeDays, ...(utmCampaign ? { utmCampaign } : {}) },
      });
      if (!res.ok)
        return toolUnavailable(tool, res.status, res.error, res.data);
      return toolOk(tool, res.data);
    ...
    case "finance.expenses.list": {
      const limit = Math.min(Math.max(asInt(args["limit"], 25), 1), 100);
      const from = asString(args["from"]);
      const to = asString(args["to"]);
      const res = await adminFetchJson(ctx, {
        path: "/api/admin/expenses",
        query: { limit, ...(from ? { from } : {}), ...(to ? { to } : {}) },
      });
      if (!res.ok)
        return toolUnavailable(tool, res.status, res.error, res.data);
      return toolOk(tool, res.data);
    ...
    case "schedule.summary": {
      ...
      const res = await adminFetchJson(ctx, {
        path: "/api/admin/schedule/summary",
        query: { range, ...(statuses ? { statuses } : {}) },
      });
      if (!res.ok)
        return toolUnavailable(tool, res.status, res.error, res.data);
      return toolOk(tool, res.data);
    ...
    case "crm.contact.snapshot": {
      ...
      const res = await adminFetchJson(ctx, {
        path: "/api/admin/contacts",
        query: { contactId, limit: 1, offset: 0 },
      });
      if (!res.ok)
        return toolUnavailable(tool, res.status, res.error, res.data);
      return toolOk(tool, res.data);
    ...
    case "crm.contact.instant_quote_photos": {
      ...
      if (!contactId) return toolError(tool, 400, "missing_contact_id");
      const res = await adminFetchJson(ctx, {
        path: `/api/admin/contacts/${encodeURIComponent(contactId)}/instant-quote-photos`,
      });
      if (!res.ok)
        return toolUnavailable(tool, res.status, res.error, res.data);
      return toolOk(tool, res.data);
    ...
    case "inbox.thread.messages": {
      ...
      const res = await adminFetchJson(ctx, {
        path: `/api/admin/inbox/threads/${encodeURIComponent(threadId)}`,
      });
      if (!res.ok)
        return toolUnavailable(tool, res.status, res.error, res.data);
    ...
    case "inbox.contact.transcript": {
      ...
      const threadLimit = Math.min(
        Math.max(asInt(args["threadLimit"], 6), 1),
        12,
      );
      const messageLimit = Math.min(
        Math.max(asInt(args["messageLimit"], 20), 1),
        80,
      );
      ...
      const threads =
        isRecord(payload) && Array.isArray(payload["threads"])
          ? (payload["threads"] as unknown[])
          : [];
      const first =
        threads.length > 0 && isRecord(threads[0]) ? threads[0] : null;
      const threadId =
      ...
      if (!threadId) {
        return {
          ...
          status: "no_data",
          httpStatus: 200,
          error: "no_threads_for_contact",
          data: { threads: [] },
        };
      }

      const transcriptRes = await runJarvisReadTool(ctx, {
        tool: "inbox.thread.messages",
        args: { threadId, limit: messageLimit },
      });
    ...
    case "google.ads.spend": {
      ...
      if (!res.ok) {
        if (res.error === "google_ads_not_configured")
          return toolNotConfigured(
            tool,
            "Google Ads not connected in StonegateOS",
          );
        return toolUnavailable(tool, res.status, res.error, res.data);
      }
      return toolOk(tool, res.data);
    ...
    case "google.ads.summary": {
      const rangeDays = Math.min(Math.max(asInt(args["rangeDays"], 7), 1), 30);
      const res = await adminFetchJson(ctx, {
        path: "/api/admin/google/ads/summary",
        query: { rangeDays },
      });
      if (!res.ok) {
        if (res.error === "google_ads_not_configured")
    ...
    case "google.ads.status": {
      const res = await adminFetchJson(ctx, {
        path: "/api/admin/google/ads/status",
      });
      if (!res.ok) {
        if (res.error === "google_ads_not_configured")
          return toolNotConfigured(
    ...
    case "google.ads.analyst.status": {
      const res = await adminFetchJson(ctx, {
        path: "/api/admin/google/ads/analyst/status",
      });
      if (!res.ok)
        return toolUnavailable(tool, res.status, res.error, res.data);
      return toolOk(tool, res.data);
    ...
    case "google.ads.analyst.recommendations": {
      ...
      if (!isRecord(payload) || !Array.isArray(payload["items"]))
        return toolOk(tool, payload);
      const filtered = (payload["items"] as unknown[]).filter(
        (item) => isRecord(item) && asString(item["status"]) === statusFilter,
      );
...
export function formatJarvisToolResultsForSystem(
  results: JarvisReadToolResult[],
): string | null {
  ...
  for (const r of results) {
    ...
    lines.push(
      `Status: ${r.status}${typeof r.httpStatus === "number" ? ` (${r.httpStatus})` : ""}${r.error ? ` - ${r.error}` : ""}`,
    );
