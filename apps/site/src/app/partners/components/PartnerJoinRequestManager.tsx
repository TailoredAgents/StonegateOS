...
function RequestCard({
  ...
  const requestedRoleAvailable = roles.some(
    (role) => role.key === request.requestedRoleKey,
  );
  ...
  const [busy, setBusy] = React.useState<
    "approve" | "needs_information" | "decline" | null
  >(null);
  ...
  async function decide(
    action: "approve" | "needs_information" | "decline",
  ): Promise<void> {
    setBusy(action);
    setNotice(null);
    const body =
      action === "approve"
        ...
        : action === "needs_information"
          ? {
              ...
              note:
                note.trim() ||
                "More information is required before access can be approved.",
            }
          : {
              action,
              note: note.trim() || "Declined by account administrator.",
            };
    const result = await partnerPortalFetch<{
      ok: true;
      joinRequest: PartnerAdminJoinRequest;
    }>(`join-requests/${encodeURIComponent(request.id)}`, {
      method: "POST",
      headers: {
    ...
    if (!result?.ok) {
      setNotice({
        ...
        text:
          result?.error.error === "conflict"
            ? "This request cannot be approved right now. Company verification, account access, or the reviewer may have changed."
            : (result?.error.message ??
              "We couldn’t record this decision. Access is unchanged."),
      });
      return;
    }
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-xs text-slate-500">
            Requested role: {request.requestedRoleKey.replaceAll("_", " ")}
          </p>
        </div>
        <span className="rounded-full bg-amber-50 px-2.5 py-1 text-xs font-semibold text-amber-900 ring-1 ring-inset ring-amber-200">
          ...
          <label className="sm:col-span-2">
            <span className="text-sm font-semibold text-slate-700">
              ...
              <span className="font-normal text-slate-500">
                (optional for approval; not emailed)
              </span>
            </span>
            <textarea
              maxLength={500}
...
export function PartnerJoinRequestManager({
  ...
  const pending = requests.filter(
    (request) => request.allowedActions.length > 0,
  );
      ...
      <div className="flex items-start gap-3">
        ...
        <div>
          <h2 className="text-lg font-semibold text-slate-950">
            Review people joining your team
          </h2>
          <p className="mt-1 text-sm leading-6 text-slate-600">
            Keep service access moving without lowering security. Approve only
            people whose verified work-email domain still matches this account;
