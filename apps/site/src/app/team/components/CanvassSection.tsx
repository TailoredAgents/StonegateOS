...
export async function CanvassSection({
  ...
  try {
    ...
    const res = await callAdminApiAs(
      principal,
      `/api/admin/canvass/queue?${qs.toString()}`,
    );
    if (res.ok) {
    ...
    <section className="space-y-6">
      <header className={TEAM_CARD_PADDED}>
        <div>
          ...
          <p className={TEAM_SECTION_SUBTITLE}>
            Door-to-door pricing flow. Create a lead with a full address, build
            a quote, and optionally schedule manual follow-ups (in-app + SMS
            reminders).
          </p>
        </div>
      </header>
        ...
        <form action={createCanvassLeadAction} className="mt-4 grid gap-4">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <span className="text-xs text-slate-500">
              Tip: after creating, build the quote below and it will draft an
              SMS for you.
            </span>
          </div>
        </form>
      ...
      {initialContactId ? (
        <div className="space-y-4">
          <div className={TEAM_CARD_PADDED}>
            ...
            <p className="mt-1 text-sm text-slate-600">
              Create a quote for this canvass lead. The system prepares an SMS
              draft after you create the quote.
            </p>
          </div>
          <QuoteBuilderSection
        ...
        <div className={TEAM_CARD_PADDED}>
          <div className={TEAM_EMPTY_STATE}>
            Create a canvass lead above to start quoting.
          </div>
        </div>
      )}
      ...
      <div className={TEAM_CARD_PADDED}>
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            <h3 className="text-base font-semibold text-slate-900">
              Canvass follow-up queue
            </h3>
            <p className="mt-1 text-sm text-slate-600">
              Manual follow-ups only. Scheduling a follow-up creates an in-app
              task and an SMS reminder for the assigned rep.
            </p>
          </div>
        </div>
        ...
        {items.length === 0 ? (
          <div className={`${TEAM_EMPTY_STATE} mt-4`}>
            No active canvass follow-ups yet.
          </div>
        ) : (
          <div className="mt-4 grid gap-4 lg:grid-cols-[minmax(0,1fr)_360px]">
            <div>
              <div className="space-y-3 lg:hidden">
                {items.map((item) => {
                  ...
                  const isActive =
                    initialContactId && item.contact.id === initialContactId;
                  return (
                    <article
                      key={item.id}
                    ...
                    >
                      <div className="flex flex-wrap items-center gap-2">
                        <span
                          className={`inline-flex rounded-full px-2.5 py-1 text-[11px] font-semibold uppercase tracking-wide ${badge.tone}`}
                        >
                          {badge.label}
                        </span>
                      ...
                      <a
                        href={quoteWorkspaceHref("create", {
                          query: { contactId: item.contact.id },
                        })}
                      ...
                      <div className="mt-4 flex flex-wrap gap-2">
                        ...
                        <form action={openContactThreadAction}>
                          ...
                          <input
                            type="hidden"
                            name="channel"
                            value={item.contact.email ? "email" : "sms"}
                          />
                          <SubmitButton
                ...
                <table className="min-w-full text-left text-xs">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {items.map((item) => {
                      ...
                      const isActive =
                        initialContactId &&
                        item.contact.id === initialContactId;
                      return (
                        <tr
                          key={item.id}
                          className={
                            isActive ? "bg-primary-50/40" : "hover:bg-slate-50"
                          }
                        >
            ...
            <aside className="rounded-2xl border border-slate-200 bg-white p-4">
              ...
              <p className="mt-1 text-xs text-slate-600">
                This creates a task + SMS reminder for the assigned rep. No
                automatic cadence runs for canvass leads.
              </p>
              <form
                action={createCanvassFollowupAction}
                className="mt-3 flex flex-col gap-2"
              >
                <input
                  type="hidden"
                  name="idempotencyKey"
                  value={`crm-reminder-create:${randomUUID()}`}
                />
