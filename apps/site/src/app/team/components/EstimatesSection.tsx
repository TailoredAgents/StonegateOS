...
const OWNER_OPTIONS = ["Austin", "Jeffrey", "Conner"];

export async function EstimatesSection(): Promise<ReactElement> {
  let payload: { ok: boolean; data: AppointmentDto[] } = {
    ok: false,
    data: [],
  };
  ...
  const upcoming = (payload.data ?? [])
    .filter((a) => a.startAt)
    .sort((a, b) =>
      a.startAt && b.startAt ? a.startAt.localeCompare(b.startAt) : 0,
    )
    .slice(0, 8);
        ...
        {columns.map((status) => (
          ...
          >
            <header className="border-b border-neutral-200 px-4 py-2">
              <h3 className="text-sm font-semibold text-primary-900 capitalize">
                {status.replace("_", " ")}
              </h3>
              <p className="text-xs text-neutral-500">
                {byStatus[status].length} appointment(s)
              </p>
            </header>
              ...
              {byStatus[status].map((a) => (
                <li key={a.id} className="px-4 py-3">
                  <p className="text-sm text-neutral-600">
                    {fmtTime(a.startAt)} <span aria-hidden="true">&bull;</span>{" "}
                    {a.contact.name}
                  </p>
                    ...
                    {status === "requested" ? (
                      <>
                        <form action={updateApptStatus}>
                          ...
                          />
                          <SubmitButton
                            className="rounded-full bg-accent-600 px-3 py-1 text-xs font-semibold text-white"
                            pendingLabel="Saving..."
                          >
                            Assign / Confirm
                          </SubmitButton>
                    ...
                    {status === "confirmed" ? (
                      <>
                        <form action={updateApptStatus}>
                          ...
                          />
                          <SubmitButton
                            className="rounded-full bg-primary-800 px-3 py-1 text-xs font-semibold text-white"
                            pendingLabel="Saving..."
                          >
                            Mark complete
                          </SubmitButton>
                        ...
                        <form action={updateApptStatus}>
                          ...
                          <SubmitButton
                            className="rounded-full border border-warning px-3 py-1 text-xs text-warning"
                            pendingLabel="Saving..."
                          >
                            No-show
                          </SubmitButton>
                  ...
                  <details className="mt-2 rounded-md border border-neutral-200 bg-neutral-50 p-3 text-xs text-neutral-700">
                    ...
                    >
                      ...
                      <SubmitButton
                        className="self-start rounded-md bg-primary-800 px-3 py-1 text-xs font-semibold text-white"
                        pendingLabel="Saving..."
                      >
                        Save note
                      </SubmitButton>
                  ...
                  <details className="mt-2 rounded-md border border-neutral-200 bg-neutral-50 p-3 text-xs text-neutral-700">
                    ...
                    <form
                      action={addApptAttachmentAction}
                      className="mt-2 flex flex-col gap-2"
                    >
                      <input type="hidden" name="appointmentId" value={a.id} />
                      <label className="flex flex-col gap-1">
                    ...
                    <details className="mt-2 rounded-md border border-neutral-200 bg-neutral-50 p-3 text-xs text-neutral-700">
                      ...
                      >
                        ...
                        <input
                          type="hidden"
                          name="services"
                          value={JSON.stringify(a.services ?? [])}
                        />
                        <label className="flex flex-col gap-1">
                          <span>Zone</span>
                          <select
                            name="zoneId"
                            defaultValue={zones[0]?.id ?? "zone-core"}
                            className="rounded-md border border-neutral-300 px-2 py-1"
                            required
                          >
                            {zones.map((zone) => (
      ...
      <section className="rounded-lg border border-primary-100 bg-primary-50/60 p-4">
        <div className="mb-2 flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-primary-700">
              Next 8 estimates with map links when available.
            </p>
          </div>
        </div>
        <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
          {upcoming.map((a) => (
            <div
              key={a.id}
              className="rounded-md border border-white/60 bg-white/90 p-3 shadow-sm"
            >
              <div className="text-[11px] font-semibold uppercase text-primary-700">
                {fmtTime(a.startAt)}
              </div>
              ...
              <div className="text-xs text-neutral-600">
                {a.property.addressLine1}, {a.property.city}, {a.property.state}{" "}
          ...
          {upcoming.length === 0 ? (
            <p className="text-xs text-neutral-600">No upcoming estimates.</p>
          ) : null}
        </div>
      </section>
    </div>
