  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white/90 p-4 shadow-lg shadow-slate-200/50">
      <div className="mb-2 flex items-center justify-between">
        ...
        <span className="text-xs text-slate-500">Top slots nearby</span>
      </div>
      {suggestions.length === 0 ? (
        ...
        <ul className="space-y-2">
          {suggestions.map((slot) => (
            <li key={slot.startAt} className="rounded-lg border border-slate-200 bg-white/80 p-3 space-y-1">
              <p className="text-sm font-semibold text-slate-900">
                {fmt(slot.startAt)} – {fmt(slot.endAt)}
              </p>
              <p className="text-xs text-slate-500">{slot.reason}</p>
              <p className="text-[11px] text-slate-600">Use this slot when booking a nearby lead to reduce drive time.</p>
            </li>
          ))}
        </ul>
      )}
