...
export function MobileInboxMediaGallery({
  messageId,
  count,
  compact = false,
}: MobileInboxMediaGalleryProps) {
  ...
  const mediaUrls = useMemo(
    () =>
      Array.from(
        { length: safeCount },
        (_, index) =>
          `/api/team/inbox/media/${encodeURIComponent(messageId)}/${index}`,
      ),
    [messageId, safeCount],
  );
  ...
  return (
    ...
    >
      ...
      {activeIndex !== null && activeMediaUrl ? (
        ...
        >
          <div
            className="w-full max-w-lg rounded-lg border border-white/10 bg-slate-950 p-3"
          ...
          >
            <div className="mb-2 flex items-center justify-between gap-2 text-xs text-slate-300">
              ...
              <button
                type="button"
                className="rounded-md border border-white/10 px-3 py-2 font-semibold text-white"
                onClick={() => setOpenIndex(null)}
              >
                Close
              </button>
            ...
            <div className="overflow-hidden rounded-md bg-slate-900">
              {activeIsVideo ? (
                <video
                  ...
                  playsInline
                  src={activeMediaUrl}
                  className="mx-auto max-h-[75dvh] w-auto max-w-full"
                />
              ) : (
                <Image
                  src={activeMediaUrl}
