...
const navItems: Array<{
  ...
  },
  {
    id: "quotes",
    label: "Quotes",
    href: "/mobile?screen=quotes",
    icon: FileText,
  },
  {
    id: "expenses",
    label: "Spend",
    href: "/mobile?screen=expenses",
    icon: ReceiptText,
  },
  {
    id: "owner",
    label: "Owner",
    href: "/mobile?screen=owner",
    icon: ShieldCheck,
  },
  {
    id: "access",
    label: "Access",
    href: "/mobile?screen=access",
    icon: UserCog,
  },
  {
    id: "settings",
    label: "More",
    href: "/mobile?screen=settings",
    icon: Settings,
  },
];
...
type OwnerHealthStatus =
  MobileOwnerSummary["health"]["providers"][number]["status"];
...
function buildMapsDirectionsHref(
...
function getProjectedEventRange(
  event: CalendarEvent,
  ...
  if (finalTotal !== null)
    return { minCents: finalTotal, maxCents: finalTotal };
...
function projectedRangeForEvents(events: CalendarEvent[]): {
  minCents: number;
  maxCents: number;
} {
...
function formatCompactProjectedRange(events: CalendarEvent[]): string {
  ...
  if (range.minCents === range.maxCents)
    return formatCompactUsdCents(range.maxCents);
...
function formatEventPricing(event: CalendarEvent): string | null {
  ...
  const finalTotal = normalizeCents(event.finalTotalCents);
  if (finalTotal !== null)
    return `Final job total ${formatUsdCents(finalTotal)}`;
  ...
  if (pricing?.mode === "range")
    return rangeLabel ? `Range ${rangeLabel}` : null;
  ...
  if (pricing?.mode === "exact")
    return exactLabel ? `Exact ${exactLabel}` : null;
...
function eventPaymentSummary(
  event: CalendarEvent,
): AppointmentPaymentSummary | null {
  return event.paymentSummary ?? null;
}
...
function attentionBadgeClass(thread: ThreadSummary): string {
  if (thread.doNotContact || thread.status === "closed")
        ...
        {days.map((dayKey) => {
          ...
          return (
            <Link
              key={dayKey}
              href={
                `/mobile?screen=${screen}&date=${encodeURIComponent(dayKey)}` as Route
              }
              className={[
            ...
            >
              <span
                className={
                  active
                    ? "block text-[10px] font-semibold uppercase text-slate-800"
                    : "block text-[10px] font-semibold uppercase text-slate-400"
                }
              >
              ...
              <span
                className={
                  active
                    ? "mt-0.5 block text-[10px] text-slate-800"
                    : "mt-0.5 block text-[10px] text-slate-500"
                }
              >
                {dayEvents.length}
                {canceledCount ? `/${canceledCount}x` : ""}
              </span>
            </Link>
          );
        })}
  ...
  return (
    <div className="space-y-3">
      ...
      {days.map((dayKey) => {
        ...
        const dayJobCount = dayEvents.filter(
          (event) =>
            event.source === "db" &&
            !isQuoteOnlyAppointmentType(event.appointmentType),
        ).length;
        ...
        return (
          <section
            key={dayKey}
            className="overflow-hidden rounded-lg border border-white/10 bg-white/[0.08]"
          >
...
async function loadMobileThreads(input: {
  view?: string;
  status?: string;
  q?: string;
  limit?: number;
  ...
  const payload = (await response
    .json()
    .catch(() => null)) as ThreadsResponse | null;
  return Array.isArray(payload?.threads) ? payload.threads : [];
}

async function loadMobileThread(
  threadId: string,
  ...
  const payload = (await response
    .json()
    .catch(() => null)) as ThreadResponse | null;
...
async function loadMobileContacts(input: {
  q: string;
...
async function loadMobileCalendarRange(
  ...
  const payload = (await response
    .json()
    .catch(() => null)) as CalendarFeedResponse | null;
  if (
    !payload ||
    payload.ok === false ||
...
async function loadMobileAccess(): Promise<{
  ...
  return {
    ...
    members: Array.isArray(membersPayload?.members)
...
export default async function MobileHomePage({
  ...
  const needsPasswordSetup =
    params.setup === "1" && !session.teamMember.passwordSet;
  const threadId =
    typeof params.threadId === "string" ? params.threadId.trim() : "";
  const contactId =
    typeof params.contactId === "string" ? params.contactId.trim() : "";
  const requestedInboxView =
    typeof params.view === "string" ? params.view.trim() : "";
  const inboxView =
    requestedInboxView === "attention" || requestedInboxView === "google"
      ? requestedInboxView
      : "all";
  const inboxStatus =
  ...
  const quoteStatus =
    ...
    quoteStatusFilters.includes(
      params.quoteStatus as (typeof quoteStatusFilters)[number],
    )
      ? params.quoteStatus
      : "all";
  ...
  const [threads, inboxCountThreads] =
    activeScreen === "inbox"
      ...
      : [[], []];
  const inboxCounts = {
    attention: inboxCountThreads.filter((thread) => thread.needsAttention)
      .length,
    google: inboxCountThreads.filter(
      (thread) =>
        thread.sourceFamily === "Google" &&
        thread.status !== "closed" &&
        !thread.doNotContact,
    ).length,
    facebook: inboxCountThreads.filter(
      (thread) =>
        thread.sourceFamily === "Facebook" &&
        thread.status !== "closed" &&
        !thread.doNotContact,
    ).length,
    due: inboxCountThreads.filter(
      (thread) => thread.attentionReason === "follow_up_due",
    ).length,
  };
  const contacts =
    activeScreen === "contacts" || activeScreen === "quotes"
      ? await loadMobileContacts({ q: inboxQuery })
      : [];
  const contactDetail =
    activeScreen === "contacts" && contactId
      ? await loadMobileContact(contactId)
      : null;
  ...
  const calendarWeekResult =
    activeScreen === "calendar" || activeScreen === "myday"
      ...
      : null;
  const calendarWeekEvents = (calendarWeekResult?.events ?? []).filter(
    (event) => calendarWeekDays.includes(eventDayKey(event)),
  );
  ...
  const teamMembers =
    activeScreen === "myday" || activeScreen === "calendar"
      ...
      : [];
  const allQuotes =
    activeScreen === "quotes" ? await loadMobileQuotes("all") : [];
  const quotes =
    quoteStatus === "all"
      ? allQuotes
  ...
  const ownerSummary =
    activeScreen === "owner" && session.isOwner
      ...
      : null;
  const accessData =
    activeScreen === "access" && session.isOwner
      ? await loadMobileAccess()
      : null;
  const canOpenMessageThreads = hasMobilePermission(
    session.teamMember.permissions,
    "messages.read",
  );
  const canStartMessageThreads = hasMobilePermission(
    session.teamMember.permissions,
    "messages.send",
  );
  ...
  const canReadPayments = hasMobilePermission(
    session.teamMember.permissions,
    "payments.read",
  );
  ...
  const selectedThread =
    activeScreen === "inbox" && threadId
      ? await loadMobileThread(threadId)
      : null;
  const selectedContact = selectedThread?.thread?.contact?.id
    ? await loadMobileContact(selectedThread.thread.contact.id)
    : null;
  const detectedThreadAddress = selectedThread
    ? detectAddressFromMessages(selectedThread.messages)
    : null;
  ...
  const quoteStatusCounts = quoteStatusFilters.reduce(
    (counts, status) => ({
      ...
      [status]:
        status === "all"
          ? allQuotes.length
          : allQuotes.filter((quote) => quote.status === status).length,
    }),
  ...
  const ownerRole =
    accessData?.roles.find((role) => role.slug === "owner") ?? null;
  const salesRole =
    accessData?.roles.find((role) => role.slug === "sales") ?? null;
    ...
    ? [
        {
          ...
          expectedRole: "owner",
          role: ownerRole,
          member: findLaunchMember(accessData.members, "Jeffrey"),
        },
        {
          name: "Austin",
          expectedRole: "owner",
          role: ownerRole,
          member: findLaunchMember(accessData.members, "Austin"),
        },
        {
          name: "Devon",
          expectedRole: "sales",
          role: salesRole,
          member: findLaunchMember(accessData.members, "Devon"),
        },
      ]
    : [];
  const mobileJobEvents = visibleTodayEvents.filter(
    (event) =>
      event.source === "db" &&
      !isQuoteOnlyAppointmentType(event.appointmentType),
  );
    ...
    <main className="min-h-dvh bg-slate-950 text-white">
      <div className="mx-auto flex min-h-dvh w-full max-w-md flex-col">
          ...
          <div className="flex items-center justify-between gap-3">
            ...
            <div className="min-w-0 rounded-md border border-white/10 bg-white/[0.08] px-3 py-2 text-right">
              <p className="truncate text-sm font-semibold">
                {session.teamMember.name}
              </p>
              <p className="text-xs text-slate-300">
                {labelForRole(session.teamMember.roleSlug)}
              </p>
            </div>
          </div>
        </header>
        ...
        <section className="flex-1 space-y-4 px-4 py-4">
          ...
          />
          {activeScreen === "inbox" ? (
            <InboxRefresh threadId={threadId} />
          ) : null}
          ...
          {needsPasswordSetup ? (
            <div className="rounded-lg border border-amber-300/30 bg-amber-300/10 p-4 text-sm text-amber-100">
              ...
              <p className="mt-1 leading-6">
                Minimum 10 characters. This lets you sign in without waiting for
                a magic link.
              </p>
              <form
                action="/mobile/password"
                method="post"
                className="mt-3 space-y-3"
              >
                ...
                <button
                  type="submit"
                  className="w-full rounded-md bg-amber-200 px-3 py-2 text-sm font-semibold text-slate-950"
                >
                  Save password
                </button>
              </form>
          ...
          {paymentReturnStatus ? (
            <div
              className={`rounded-lg border p-4 text-sm ${
                paymentReturnStatus === "verified"
                  ? "border-emerald-300/30 bg-emerald-300/10 text-emerald-100"
                  : paymentReturnStatus === "needs_review" ||
          ...
          {activeScreen === "inbox" ? (
              ...
              {selectedThread?.thread ? (
                <div className="rounded-lg border border-white/10 bg-white/[0.08]">
                  <div className="border-b border-white/10 p-4">
                    <Link
                      href={
                        `/mobile?view=${encodeURIComponent(inboxView)}` as Route
                      }
                    ...
                    <div className="mt-3 flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        <h2 className="truncate text-lg font-semibold">
                          {selectedThread.thread.contact?.name ?? "Contact"}
                        </h2>
                      ...
                      (selectedThread.thread.contact.phone ||
                        ...
                        <form action={startMobileContactCallAction}>
                          ...
                          <input
                            type="hidden"
                            name="threadId"
                            value={selectedThread.thread.id}
                          />
                          <input
                            type="hidden"
                            name="returnTo"
                            value={`/mobile?threadId=${encodeURIComponent(threadId)}`}
                          />
                          <button
                            type="submit"
                    ...
                    <div className="mt-3 flex flex-wrap items-center gap-2">
                      {selectedThread.thread.doNotContact ? (
                        <span className="rounded-full bg-rose-300/15 px-2.5 py-1 text-xs font-semibold text-rose-100">
                          DNC
                        </span>
                      ) : null}
                      ...
                      <form action={markMobileThreadHandledAction}>
                        ...
                        <button
                          type="submit"
                          className="rounded-md border border-white/10 bg-slate-900 px-3 py-2 text-xs font-semibold text-slate-200"
                        >
                          Mark handled
                        </button>
                      </form>
                      <details className="relative">
                        ...
                        <div className="absolute right-0 z-20 mt-2 w-64 rounded-md border border-white/10 bg-slate-950 p-3 shadow-xl">
                          <form
                            action={closeMobileThreadAction}
                            className="space-y-2"
                          >
                            <input
                              type="hidden"
                            ...
                            <button
                              name="closeReason"
                              value="lost"
                              className="w-full rounded-md border border-white/10 bg-slate-900 px-3 py-2 text-left text-sm font-semibold text-slate-100"
                            >
                              Lost
                            </button>
                            <label className="block rounded-md border border-white/10 bg-slate-900 p-2">
                              ...
                              <button
                                ...
                                className="mt-2 w-full rounded-md border border-rose-300/30 bg-rose-300/10 px-3 py-2 text-left text-sm font-semibold text-rose-100"
                              >
                                Do Not Contact
                              </button>
                            </label>
                  ...
                  {selectedThreadMediaMessages.length ? (
                    <div className="border-b border-white/10 p-4">
                      <div className="flex items-center justify-between gap-3">
                        ...
                        <span className="rounded-full bg-slate-800 px-2.5 py-1 text-xs font-semibold text-slate-300">
                          {selectedThreadMediaMessages.reduce(
                            (sum, message) =>
                              sum + (message.mediaUrls?.length ?? 0),
                            0,
                          )}
                        </span>
                      </div>
                      <div className="mt-3 space-y-3">
                        {selectedThreadMediaMessages
                          .slice(-8)
                          .map((message) => (
                            <MobileInboxMediaGallery
                              key={`strip-${message.id}`}
                              messageId={message.id}
                              count={message.mediaUrls?.length ?? 0}
                              compact
                            />
                          ))}
                      </div>
                ...
                <div className="rounded-lg border border-white/10 bg-white/[0.08] p-4">
                  ...
                  <div className="mt-4 grid grid-cols-3 gap-2">
                    {[
                      ...
                      const href =
                        `/mobile?view=${view.id}${inboxQuery ? `&q=${encodeURIComponent(inboxQuery)}` : ""}` as Route;
                      return (
                        <Link
                          key={view.id}
                          href={href}
                  ...
                  <div className="mt-4 space-y-2">
                    {threads.length > 0 ? (
                      threads.map((thread) => {
                        ...
                        const threadHref =
                          `/mobile?threadId=${encodeURIComponent(thread.id)}&view=${encodeURIComponent(inboxView)}` as Route;
                        return (
                          <div
                            key={thread.id}
                              ...
                              <div className="flex items-start justify-between gap-3">
                                <div className="min-w-0">
                                  <div className="flex min-w-0 flex-wrap items-center gap-1.5">
                                    ...
                                    <span
                                      className={`rounded-full border px-2 py-0.5 text-[10px] font-semibold ${sourceBadgeClass(source)}`}
                                    >
                                      {source}
                                    </span>
                                  </div>
                                ...
                                <div className="shrink-0 text-right">
                                  <span
                                    className={`rounded-full px-2 py-0.5 text-[10px] font-semibold ${attentionBadgeClass(thread)}`}
                                  >
                            ...
                            {phone && thread.contact?.id ? (
                              ...
                              >
                                ...
                                <input
                                  type="hidden"
                                  name="returnTo"
                                  value={`/mobile?view=${encodeURIComponent(inboxView)}`}
                                />
                                <button
                                  type="submit"
                                  className="w-full rounded-md border border-cyan-300/40 bg-cyan-300/10 px-3 py-2 text-sm font-semibold text-cyan-100"
                                >
                                  Call
                                </button>
                              </form>
              ...
              {selectedContact ? (
                <div className="rounded-lg border border-white/10 bg-white/[0.08] p-4">
                  ...
                  <div className="mt-4 grid grid-cols-2 gap-2">
                    <div className="rounded-md border border-white/10 bg-slate-900 p-3">
                      ...
                      <p className="mt-1 text-xl font-semibold">
                        {selectedContact.stats?.appointments ?? 0}
                      </p>
                    </div>
                    <div className="rounded-md border border-white/10 bg-slate-900 p-3">
                      <p className="text-xs text-slate-400">Quotes</p>
                      <p className="mt-1 text-xl font-semibold">
                        {selectedContact.stats?.quotes ?? 0}
                      </p>
                    </div>
                  </div>
                    ...
                    <div className="mt-4">
                      ...
                      <div className="mt-2 space-y-2">
                        {selectedContact.notes.slice(0, 3).map((note) => (
                          <div
                            key={note.id}
                            className="rounded-md border border-white/10 bg-slate-900 p-3 text-sm leading-6 text-slate-200"
                          >
                            <p>{note.body}</p>
                            <p className="mt-1 text-xs text-slate-400">
                  ...
                  <details className="mt-4 rounded-md border border-white/10 bg-slate-900 p-3">
                    ...
                    <form
                      action={updateMobileContactAction}
                      className="mt-3 space-y-3"
                    >
                      <input
                        type="hidden"
                  ...
                  <details className="mt-3 rounded-md border border-white/10 bg-slate-900 p-3">
                    ...
                    <form
                      action={bookMobileAppointmentAction}
                      className="mt-3 space-y-3"
                    >
                      <input
                        type="hidden"
                      ...
                      <MobileAppointmentPricingFields
                        sourceTeamMemberId={session.teamMember.id}
                      />
                      <label className="block">
                        <span className="text-xs font-semibold text-slate-300">
                          Notes
                      ...
                      <button
                        type="submit"
                        className="w-full rounded-md bg-cyan-300 px-4 py-3 text-sm font-semibold text-slate-950"
                      >
                        Book appointment
                      </button>
                    </form>
                  ...
                  <form
                    action={addMobileContactNoteAction}
              ...
              {contactDetail ? (
                <div className="rounded-lg border border-white/10 bg-white/[0.08] p-4">
                  ...
                  <div className="mt-4 grid grid-cols-2 gap-2">
                    <div className="rounded-md border border-white/10 bg-slate-900 p-3">
                      <p className="text-xs text-slate-400">Appointments</p>
                      <p className="mt-1 text-xl font-semibold">
                        {contactDetail.stats?.appointments ?? 0}
                      </p>
                    </div>
                    <div className="rounded-md border border-white/10 bg-slate-900 p-3">
                      <p className="text-xs text-slate-400">Quotes</p>
                      <p className="mt-1 text-xl font-semibold">
                        {contactDetail.stats?.quotes ?? 0}
                      </p>
                    </div>
                  </div>
                ...
                <div className="rounded-lg border border-white/10 bg-white/[0.08] p-4">
                  ...
                  <form action="/mobile" className="mt-4">
                    ...
                    <button
                      type="submit"
                      className="mt-3 w-full rounded-md bg-cyan-300 px-4 py-3 text-sm font-semibold text-slate-950"
                    >
                      Search contacts
                    </button>
                  </form>
                    ...
                    {contacts.length > 0 ? (
                      contacts.map((contact) => (
                        <Link
                          key={contact.id}
                          href={
                            `/mobile?screen=contacts&contactId=${encodeURIComponent(contact.id)}` as Route
                          }
              ...
              <div className="rounded-lg border border-white/10 bg-white/[0.08] p-4">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-slate-300">
                      {quoteStatusCounts.pending + quoteStatusCounts.sent} open
                      worth {formatUsdDollars(openQuoteValue)}
                    </p>
                  </div>
                  <span className="rounded-full bg-slate-800 px-2.5 py-1 text-xs font-semibold text-slate-300">
                    {quotes.length}
                ...
                <div className="mt-4 grid grid-cols-3 gap-2">
                  {quoteStatusFilters.map((status) => (
                    <Link
                      key={status}
                      href={
                        `/mobile?screen=quotes${status === "all" ? "" : `&quoteStatus=${status}`}` as Route
                      }
                    ...
                    return (
                      ...
                      >
                        ...
                        <p className="mt-3 text-sm text-slate-300">
                          {quote.services.map(formatStage).join(", ")}
                        </p>
                        {quote.notes ? (
                        ...
                        <div className="mt-3 grid grid-cols-2 gap-2 text-xs text-slate-400">
                          <div className="rounded-md border border-white/10 bg-slate-900 px-3 py-2">
                            ...
                            <p className="mt-0.5 font-semibold text-slate-200">
                              {formatRelativeTime(quote.updatedAt) || "Unknown"}
                            </p>
                          </div>
                          <div className="rounded-md border border-white/10 bg-slate-900 px-3 py-2">
                            ...
                            <p className="mt-0.5 font-semibold text-slate-200">
                              {quote.sentAt
                                ? formatRelativeTime(quote.sentAt)
                                : quote.expiresAt
                          ...
                          <div className="rounded-md border border-white/10 bg-slate-900 px-3 py-2">
                            ...
                            <p className="mt-0.5 font-semibold text-slate-200">
                              {quote.viewedAt
                                ? `${quote.viewCount}x ${formatRelativeTime(quote.lastViewedAt)}`
                                : "No"}
                            </p>
                          </div>
                          <div className="rounded-md border border-white/10 bg-slate-900 px-3 py-2">
                            ...
                            <p className="mt-0.5 font-semibold text-slate-200">
                              {quote.acceptedAppointmentId
                                ...
                                : quote.refreshRequestedAt
                                  ? "Refresh requested"
                                  : `${Math.round(((quote.jobDurationMinutes ?? 120) / 60) * 10) / 10}h`}
                            </p>
                          </div>
                        </div>
                        ...
                        quote.status === "sent" ? (
                          <details className="mt-3 rounded-md border border-white/10 bg-slate-900 p-3">
                            ...
                            <form
                              action={updateMobileQuoteAction}
                              className="mt-3 space-y-3"
                            >
                              <input
                                type="hidden"
                              ...
                              <div className="space-y-2">
                                {mobileQuoteServices.map((service) => {
                                  const checked = quote.services.includes(
                                    service.id,
                                  );
                                  return (
                                    <div
                                    ...
                                    >
                                      ...
                                      <input
                                        name={`servicePrice:${service.id}`}
                                        inputMode="decimal"
                                        defaultValue={quoteServiceAmount(
                                          quote,
                                          service.id,
                                        )}
                                        className="w-full rounded-md border border-white/10 bg-slate-900 px-2 py-1 text-sm text-white outline-none focus:border-cyan-300"
              ...
              <div className="rounded-lg border border-white/10 bg-white/[0.08] p-4">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    ...
                    <h2 className="mt-1 text-lg font-semibold">
                      {formatDateLabel(calendarWeekDays[0] ?? calendarDay)} -{" "}
                      {formatDateLabel(calendarWeekDays[6] ?? calendarDay)}
                    </h2>
                  </div>
                  <div className="text-right">
                    <p className="text-xs text-slate-400">Week projected</p>
                    <p className="text-xl font-semibold text-cyan-200">
                      {calendarWeekProjectedLabel}
                    </p>
                  </div>
                </div>
                <div className="mt-4 grid grid-cols-3 gap-2">
                  <Link
                    href={
                      `/mobile?screen=calendar&date=${encodeURIComponent(addDaysToKey(calendarDay, -7))}` as Route
                    }
                    className="rounded-md border border-white/10 bg-slate-900 px-3 py-2 text-center text-sm font-semibold text-slate-200"
                  >
                    Prev wk
                ...
                >
                  ...
                  <button
                    type="submit"
                    className="rounded-md border border-white/10 bg-slate-900 px-3 py-2 text-sm font-semibold text-slate-200"
                  >
                    Go
                  </button>
                </form>
            ...
            <div className="space-y-4">
              <div className="rounded-lg border border-white/10 bg-white/[0.08] p-4">
                ...
                <h2 className="mt-1 text-lg font-semibold">
                  {formatDateLabel(ownerSummary.todayKey)}
                </h2>
                <p className="mt-1 text-sm text-slate-300">
                  Completed job revenue, payments, projected work, payouts,
                  leads, and provider health.
                </p>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="rounded-lg border border-emerald-300/20 bg-emerald-300/10 p-4">
                  ...
                  <p className="mt-2 text-2xl font-semibold">
                    {formatUsdCents(ownerSummary.collectedTodayCents)}
                  </p>
                  <p className="mt-1 text-xs text-emerald-100">
                    {ownerSummary.collectedTodayCount} completed jobs
                  </p>
                </div>
                <div className="rounded-lg border border-cyan-300/20 bg-cyan-300/10 p-4">
                  ...
                  <p className="mt-2 text-2xl font-semibold">
                    {formatUsdCents(ownerSummary.projectedTodayCents)}
                  </p>
                  <p className="mt-1 text-xs text-cyan-100">
                    {ownerSummary.bookedJobsToday} booked jobs today
                  </p>
                </div>
                <div className="rounded-lg border border-white/10 bg-white/[0.08] p-4">
                  ...
                  <p className="mt-2 text-2xl font-semibold">
                    {formatUsdCents(ownerSummary.collectedWeekCents)}
                  </p>
                  <p className="mt-1 text-xs text-slate-400">
                    {ownerSummary.collectedWeekCount} completed jobs
                  </p>
                </div>
                <div className="rounded-lg border border-white/10 bg-white/[0.08] p-4">
                  <p className="text-xs font-semibold uppercase tracking-[0.14em] text-slate-300">
                  ...
                  <p className="mt-2 text-2xl font-semibold">
                    {formatUsdCents(ownerSummary.collectedMonthCents)}
                  </p>
                  <p className="mt-1 text-xs text-slate-400">
                    {ownerSummary.collectedMonthCount} completed jobs
                  </p>
                </div>
                <Link
                  href="/mobile"
                ...
                <div className="rounded-lg border border-white/10 bg-white/[0.08] p-4">
                  ...
                  <p className="mt-2 text-2xl font-semibold">
                    {formatUsdCents(ownerSummary.collectedLast30DaysCents)}
                  </p>
                  <p className="mt-1 text-xs text-slate-400">
                    {ownerSummary.collectedLast30DaysCount} completed jobs
                  </p>
                </div>
                <div className="rounded-lg border border-white/10 bg-white/[0.08] p-4">
                  <p className="text-xs font-semibold uppercase tracking-[0.14em] text-slate-300">
                  ...
                  <p className="mt-2 text-2xl font-semibold">
                    {formatUsdCents(ownerSummary.collectedYearCents)}
                  </p>
                  <p className="mt-1 text-xs text-slate-400">
                    {ownerSummary.collectedYearCount} completed jobs
                  </p>
                </div>
                <div className="rounded-lg border border-emerald-300/20 bg-emerald-300/10 p-4">
                  <p className="text-xs font-semibold uppercase tracking-[0.14em] text-emerald-200">
                    Lifetime Job Revenue
                  </p>
                  <p className="mt-2 text-2xl font-semibold">
                    {formatUsdCents(ownerSummary.collectedLifetimeCents)}
                  </p>
                  <p className="mt-1 text-xs text-emerald-100">
                    {ownerSummary.collectedLifetimeCount} completed jobs
                  </p>
                </div>
              </div>

              {ownerSummary.paymentLedger ? (
                <div className="rounded-lg border border-white/10 bg-white/[0.08] p-4">
                  <div>
                    <h2 className="text-base font-semibold">Payment Ledger</h2>
                    <p className="mt-1 text-sm text-slate-300">
                      Verified collection activity, kept separate from completed
                      job revenue.
                    </p>
                  </div>
                  <div className="mt-3 grid grid-cols-2 gap-2 text-xs">
                    <div className="rounded-md border border-emerald-300/20 bg-emerald-300/10 p-3">
                      <p className="text-emerald-100">Payments collected</p>
                      <p className="mt-1 text-lg font-semibold text-white">
                        {formatUsdCents(
                          ownerSummary.paymentLedger.paymentsCollectedCents,
                        )}
                      </p>
                    </div>
                    <div className="rounded-md border border-white/10 bg-slate-900 p-3">
                      <p className="text-slate-400">Toward jobs</p>
                      <p className="mt-1 text-lg font-semibold text-slate-100">
                        {formatUsdCents(
                          ownerSummary.paymentLedger.paidTowardJobsCents,
                        )}
                      </p>
                    </div>
                    <div className="rounded-md border border-amber-300/20 bg-amber-300/10 p-3">
                      ...
                      <p className="mt-1 text-lg font-semibold text-white">
                        {formatUsdCents(
                          ownerSummary.paymentLedger.outstandingBalanceCents,
                        )}
                      </p>
                    </div>
                    <div className="rounded-md border border-white/10 bg-slate-900 p-3">
                      <p className="text-slate-400">Tips collected</p>
                      <p className="mt-1 text-lg font-semibold text-slate-100">
                        {formatUsdCents(
                          ownerSummary.paymentLedger.tipsCollectedCents,
                        )}
                      </p>
                    </div>
                    <div className="rounded-md border border-white/10 bg-slate-900 p-3">
                      ...
                      <p className="mt-1 text-lg font-semibold text-slate-100">
                        {formatUsdCents(
                          ownerSummary.paymentLedger.refundedCents,
                        )}
                      </p>
                    </div>
                    <div className="rounded-md border border-rose-300/20 bg-rose-300/10 p-3">
                      <p className="text-rose-100">Needs review</p>
                      <p className="mt-1 text-lg font-semibold text-white">
                        {formatUsdCents(
                          ownerSummary.paymentLedger.needsReviewCents,
                        )}
                      </p>
                      <p className="mt-1 text-rose-100">
                        {ownerSummary.paymentLedger.needsReviewCount} records
                      </p>
                    </div>
                  </div>
                </div>
              ...
              <div className="rounded-lg border border-white/10 bg-white/[0.08] p-4">
                <div className="flex items-start justify-between gap-3">
                  ...
                  <form action={runMobilePayoutAction}>
                    ...
                    <button
                      type="submit"
                      className="rounded-md border border-cyan-300 bg-cyan-300 px-3 py-2 text-xs font-semibold text-slate-950"
                    >
                      Create
                    </button>
                  </form>
                  ...
                  <div className="mt-3 grid grid-cols-2 gap-2 text-xs">
                    <div className="rounded-md border border-white/10 bg-slate-900 p-3">
                      ...
                      <p className="mt-1 font-semibold text-slate-100">
                        {formatUsdCents(
                          ownerSummary.currentPayout.totalsCents.sales,
                        )}
                      </p>
                    </div>
                    <div className="rounded-md border border-white/10 bg-slate-900 p-3">
                      ...
                      <p className="mt-1 font-semibold text-slate-100">
                        {formatUsdCents(
                          ownerSummary.currentPayout.totalsCents.crew,
                        )}
                      </p>
                    </div>
                    <div className="rounded-md border border-white/10 bg-slate-900 p-3">
                      ...
                      <p className="mt-1 font-semibold text-slate-100">
                        {formatUsdCents(
                          ownerSummary.currentPayout.totalsCents.marketing,
                        )}
                      </p>
                    </div>
                    <div className="rounded-md border border-white/10 bg-slate-900 p-3">
                      ...
                      <p className="mt-1 font-semibold text-slate-100">
                        {formatUsdCents(
                          ownerSummary.currentPayout.cardTipsCents,
                        )}
                      </p>
                    </div>
                  </div>
                      ...
                      >
                        ...
                        <div className="mt-3 grid grid-cols-2 gap-2 text-xs">
                          <span className="rounded-md border border-white/10 px-2 py-1 text-slate-300">
                            Reimbursements{" "}
                            {formatUsdCents(run.reimbursementTotalCents)}
                          </span>
                          <span className="rounded-md border border-white/10 px-2 py-1 text-slate-300">
                            Adjustments{" "}
                            {formatUsdCents(run.otherAdjustmentsTotalCents)}
                          </span>
                        </div>
                        <div className="mt-3 grid grid-cols-2 gap-2">
                          <a
                          ...
                          {run.status === "locked" ? (
                            <form action={runMobilePayoutAction}>
                              ...
                              <button
                                type="submit"
                                className="w-full rounded-md border border-emerald-300 bg-emerald-300 px-3 py-2 text-sm font-semibold text-slate-950"
                              >
                                Mark Paid
                              </button>
                            </form>
                  ...
                  {ownerSummary.health.providers.length > 0 ? (
                    ownerSummary.health.providers.map((provider) => (
                      <div
                        ...
                        className={`rounded-md border px-3 py-2 text-xs font-semibold ${ownerHealthClass(provider.status)}`}
                      >
                    ...
                    <p className="col-span-2 text-sm text-slate-300">
                ...
                <div className="mt-3 space-y-2">
                  {ownerSummary.nextAppointments.length > 0 ? (
                    ownerSummary.nextAppointments.map((appointment) => {
                      const mapsHref = buildMapsDirectionsHref(
                        appointment.address,
                      );
                      return (
                        <div
                        ...
                        >
                          <div className="flex items-start justify-between gap-3">
                            <div className="min-w-0">
                              <p className="truncate text-sm font-semibold">
                                {appointment.contactName ?? appointment.title}
                              </p>
                              <p className="mt-1 text-xs text-slate-400">
                                {appointment.time}
                              </p>
                            ...
                            <span className="shrink-0 text-sm font-semibold text-cyan-100">
                              {formatUsdCents(appointment.projectedCents)}
                            </span>
                          </div>
                        </div>
                      );
                  ...
                  {ownerSummary.inboxLeads.length > 0 ? (
                    ownerSummary.inboxLeads.map((lead) => (
                      <Link
                        key={lead.id}
                        href={
                          `/mobile?threadId=${encodeURIComponent(lead.id)}` as Route
                        }
                        className="block rounded-md border border-white/10 bg-slate-900 p-3"
                      >
                        <div className="flex items-center justify-between gap-3">
            ...
            <div className="space-y-4">
              <div className="rounded-lg border border-white/10 bg-white/[0.08] p-4">
                ...
                <p className="mt-1 text-sm leading-6 text-slate-300">
                  Confirm Jeffrey and Austin are owners, Devon is sales, and
                  each person has an active account.
                </p>
              </div>
                ...
                {launchAccounts.map((entry) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {member ? (
                        <form
                          action={updateMobileTeamMemberAction}
                          className="mt-4 space-y-3"
                        >
                          <input
                            type="hidden"
                          ...
                          <div className="grid grid-cols-2 gap-2 text-xs">
                            ...
                            <div className="rounded-md border border-white/10 bg-slate-900 px-3 py-2">
                              ...
                              <p className="mt-1 font-semibold">
                                {member.passwordSet ? "Set" : "Not set"}
                              </p>
                            </div>
                          </div>
                          <label className="flex items-center gap-2 text-sm text-slate-200">
                      ...
                      {member?.email || member?.phone ? (
                        <form
                          action={sendMobileTeamInviteAction}
                          className="mt-3"
                        >
                          <input
                            type="hidden"
                    ...
                    >
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0">
                          ...
                          <p className="mt-1 truncate text-xs text-slate-400">
                            {member.email ?? member.phone ?? "No login contact"}
                          </p>
                        </div>
                ...
                <div className="mt-4 space-y-2 text-sm">
                  ...
                  <div className="flex items-center justify-between gap-3 rounded-md border border-white/10 bg-slate-900 px-3 py-2">
                    ...
                    <span className="truncate font-semibold">
                      {session.teamMember.email ?? "Not set"}
                    </span>
                  </div>
                  <div className="flex items-center justify-between gap-3 rounded-md border border-white/10 bg-slate-900 px-3 py-2">
                    <span className="text-slate-400">Password</span>
                    <span className="font-semibold">
                      {session.teamMember.passwordSet ? "Set" : "Not set"}
                    </span>
                  </div>
                  <div className="flex items-center justify-between gap-3 rounded-md border border-white/10 bg-slate-900 px-3 py-2">
              ...
              <div className="rounded-lg border border-white/10 bg-white/[0.08] p-4">
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  {session.allowedScreens.map((screen) => (
                    <span
                      key={screen}
                      className="rounded-full border border-white/10 bg-slate-900 px-3 py-1 text-xs font-semibold capitalize text-slate-200"
                    >
                      {screen}
                    </span>
                  ))}
            ...
            <div className="rounded-lg border border-white/10 bg-white/[0.08] p-4">
              ...
              <div className="mt-4 rounded-md border border-dashed border-white/15 bg-slate-900 p-4 text-sm leading-6 text-slate-300">
                This mobile screen is planned for a later milestone. Inbox is
