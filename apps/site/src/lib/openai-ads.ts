import { isAdvertisingAllowed, isPublicTrackingPath } from "./cookie-consent";
...
export function isOpenAiAdsPublicPath(pathname: string): boolean {
  return isPublicTrackingPath(pathname);
}
...
export function isOpenAiAdsMeasurementAllowed(): boolean {
  if (typeof window === "undefined" || !isAdvertisingAllowed() || hasOpenAiAdsPrivacySignal())
    return false;
  const preference =
    consentOverride ??
