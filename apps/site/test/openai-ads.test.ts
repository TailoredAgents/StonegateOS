...
import { COOKIE_CONSENT_COOKIE } from "../src/lib/cookie-consent";
import {
...
void test("legacy opt-in and configuration cannot bypass missing shared advertising consent", () => {
  const b = browser("https://example.com/book?oppref=unconsented-click");
  b.cookies.delete(COOKIE_CONSENT_COOKIE);
  process.env["NEXT_PUBLIC_OPENAI_ADS_REQUIRE_CONSENT"] = "false";
  setOpenAiAdsMeasurementConsent(true);
  assert.equal(isOpenAiAdsMeasurementAllowed(), false);
  assert.equal(initializeOpenAiAdsPixel("pixel"), false);
  assert.deepEqual(getOpenAiAdsAttribution(), { consent: false });
  assert.equal(b.scripts.length, 0);
  assert.equal(b.storage.has("sg:openai-ads-attribution"), false);
});
