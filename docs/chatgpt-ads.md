...
| Site           | `NEXT_PUBLIC_OPENAI_ADS_REQUIRE_CONSENT` | `true`. Shared website Advertising consent is always required, regardless of this legacy flag.             |
...
Public pages now use a shared cookie choice with Analytics and Advertising
categories. Optional tracking starts disabled for every visitor. Accepting
Advertising enables configured Google Ads, Meta, and OpenAI measurement;
rejecting it keeps these tags and ad attribution disabled. The footer and
`/privacy#cookies` provide Cookie settings. GPC and DNT override both optional
categories. The versioned choice is stored for 180 days. Withdrawing Advertising
also invokes the existing OpenAI server revocation flow; it is separate from
permission to contact the customer about a requested service.
